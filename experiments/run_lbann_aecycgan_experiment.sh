...
### 3 model JAG conduit data with autoencoder
MODEL_PROTO="--model={${LBANN_DIR}/model_zoo/models/jag/ae_cycle_gan/3models/ae.prototext,${LBANN_DIR}/model_zoo/models/jag/ae_cycle_gan/3models/cycle_gan.prototext,${LBANN_DIR}/model_zoo/models/jag/ae_cycle_gan/3models/ae_cyc.prototext}"
...
READER_PROTO="--reader=${LBANN_DIR}/model_zoo/models/jag/ae_cycle_gan/data_reader_train1M_Atest1M_B.prototext"
...
NUM_NODES=8      # default: number of allocated nodes (1 if none)
PROCS_PER_NODE= # default: GPUs per node (2 if cluster has no GPUs)
CLUSTER=
PARTITION=
ACCOUNT=
TIME_LIMIT=24:00     # default: 1:00 (format is hours:minutes)
...
LBANN_EXE="${LBANN_DIR}/build/gnu.${BUILD}.${CLUSTER}.llnl.gov/lbann/build/model_zoo/lbann_aecycgan"
case ${USE_GPU} in
    YES|yes|TRUE|true|ON|on|1)
        case ${HAS_GPU} in
            YES|yes|TRUE|true|ON|on|1)
