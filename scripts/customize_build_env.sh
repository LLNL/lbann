...
#                MODULE_CMD_CLANG="module load gcc/10.3.1 cuda/11.8.0 mvapich2/2.3.7 python/3.9.12"
            ...
            "ivybridge" | "cascadelake") # Catalyst, Ruby
                ...
                ;;
            "zen" | "zen2") # Corona
                MODULE_CMD="module load StdEnv clang/14.0.6-magic openmpi/4.1.2 git/2.36.1 cmake/3.26.3 emacs/28.2 rocm/5.7.1"
...
#                DEPENDENTS_CENTER_COMPILER="%gcc@10.3.1"
            ...
            "zen" | "zen2") # Corona
                ...
                CENTER_DEPENDENCIES="^openmpi@4.1.2 ^hip@5.7.1 ^python@3.9.12 ^py-protobuf@4.21.5"
                CENTER_PIP_PACKAGES="${LBANN_HOME}/scripts/common_python_packages/requirements.txt ${LBANN_HOME}/ci_test/requirements.txt"
                ;;
...
EOF
        ...
        set_superbuild_externals ${host} "rocm-5.7.1" "clang-14.0.6-magic" "openmpi-4.1.2" "$yaml" "${prefix}"
        set_superbuild_DHA_externals ${host} "rocm-5.7.1" "clang-14.0.6-magic" "openmpi-4.1.2" "$yaml" "${prefix}" "${dha_dir}"
