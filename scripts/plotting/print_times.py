...
def print_layer_times(filename, only_model = -1):
  ...
  results = load_events.load_values(
    ...
    layer_event_names=['fp_time', 'bp_time', 'update_time', 'opt_time'],
    model=-1)
  for model in results.keys():
    ...
    for layer in results[model]['fp_time'].keys():
      ...
      opt_tot += l_opt_tot
      print('Layer {0}:\tfp={1:<10.4}\tbp={2:<10.4}\tupdate={3:<10.4}\topt={4:<10.4}%'.format(
        layer, fp_mean, bp_mean, update_mean, opt_mean))
      print(' '*len('layer {0}'.format(layer)) +
            ':\tfp={0:<10.4}\tbp={1:<10.4}\tupdate={2:<10.4}\topt={3:<10.4}%'.format(
              l_fp_tot, l_bp_tot, l_update_tot, l_opt_tot))
    print('Total: fp={0:.4} bp={1:.4} update={2:.4} opt={3:.4} %'.format(
      fp_tot, bp_tot, update_tot, opt_tot))
