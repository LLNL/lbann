 ...
 * the pipeline.
 */
void setup_preprocessor(const params& mp,
                        lbann::cv_process& pp,
                        int& mean_extractor_idx)
{
  ...
  if (cp.m_is_set) { // If cropper parameters are given
    ...
    cropper->set(cp.m_crop_sz.first,
                 cp.m_crop_sz.second,
                 cp.m_rand_center,
                 cp.m_roi_sz);
  ...
  if (mp.to_enable_decolorizer()) { // Set up a decolorizer
    std::unique_ptr<lbann::cv_decolorizer> decolorizer(
      new (lbann::cv_decolorizer));
  ...
  if (mp.to_enable_mean_extractor()) { // set up a mean extractor
    ...
    std::unique_ptr<lbann::cv_mean_extractor> mean_extractor(
      new (lbann::cv_mean_extractor));
    if (cp.m_is_set) {
      mean_extractor->set(cp.m_crop_sz.first,
                          cp.m_crop_sz.second,
                          n_ch,
                          mp.get_mean_batch_size());
    }
    else {
...
/**
 * This is the main processing loop that crops each image and accumulates its
 * pixel values. Finally, it computes the pixel-wise mean of cropped images and
 * stores it.
 */
                    ...
                    walltimes& wt)
{
  ...
  const float progress_report_term =
    ((filenames.size() * mp.get_report_freq() < 1.0)
       ? 1.0
       : (filenames.size() * mp.get_report_freq()));
  ...
  for (size_t i = 0u; i < filenames.size(); ++i) {
    ...
    cv::Mat image =
      cv::imdecode(inbuf, cv::IMREAD_ANYCOLOR | cv::IMREAD_ANYDEPTH);
    ...
    if (mp.to_write_cropped()) { // Export the cropped image
      ...
      const size_t capacity =
        lbann::image_data_amount(image) + max_img_header_size;
      if (outbuf.size() < capacity) {
        // std::cout << "bytes reserved for the image: " <<
        // image_data_amount(image) << std::endl;
      ...
      std::string ofilename =
        img_list.get_image_name_with_new_ext(i, mp.get_out_ext());
    ...
    if (ms.is_root()) {
      if (i >= static_cast<size_t>(next_progress_report_point + 0.5)) {
        ...
        std::string current_progress =
          "Progress " +
          std::to_string(static_cast<int>(100.0f * i / filenames.size())) + '%';
  ...
  if (mp.to_enable_mean_extractor()) {
    ...
    bool ok =
      write_mean_image(pp, mean_extractor_idx, ms, img_list.get_out_dir());
    if (!ok) {
