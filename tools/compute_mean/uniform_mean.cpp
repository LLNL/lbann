...
#include "lbann/data_ingestion/readers/cv_subtractor.hpp"
...
/**
 * Scale the mean pixel to the depth of the output image with the adjustment for
 * rounding. Convert to the unit scale if to_unit_scale is set when in_depth is
 * of integral and out_depth is of float.
 */
cv::Scalar
scale_pixel(const cv::Scalar pixel_m, const int in_depth, const int out_depth)
{
  const double alpha = lbann::get_depth_denormalizing_factor(out_depth) *
                       lbann::get_depth_normalizing_factor(in_depth);

  const double rounding_off_compensation =
    (lbann::is_float(out_depth) ? 0.0 : 0.5);
  cv::Scalar pixel(0.0);
 ...
 */
void write_bin_image(const int height,
                     const int width,
                     const int cv_type,
                     const cv::Scalar pixel)
{
  cv::Mat mean_image(height, width, cv_type, pixel);
  std::string image_name = "mean_uniform-" + std::to_string(mean_image.cols) +
                           'x' + std::to_string(mean_image.rows) + 'x' +
                           std::to_string(mean_image.channels()) + '-' +
                           std::to_string(mean_image.depth()) + ".bin";

  const size_t sz = mean_image.rows * mean_image.cols * mean_image.channels() *
                    CV_ELEM_SIZE(mean_image.depth());
...
{
  std::cout
    << "Create a mean image that is filled with the same pixel value given "
    << std::endl
    << "or computed from a given image (the channel-wise mean)." << std::endl
    << "Usage: > " << cmd << " width height B G R [depth] (for a color image)"
    << std::endl
    << "    or > " << cmd << " width height M [depth] (for a monochrome image)"
    << std::endl
    << "    or > " << cmd << " input_image [depth]" << std::endl
    << " - B, G, R, M: mean channel values" << std::endl
    << " - depth: OpenCV depth code [0|2]"
    << std::endl
...
int main(int argc, char** argv)
{
  ...
  int in_depth = CV_8U; // The depth of the input pixel. The default is unsigned
                        // 8-bit [0 255]
  cv::Mat mean_image;   // The mean image to output
  cv::Scalar pixel_m(0.0); // The mean pixel value (in double precision)
  cv::Scalar pixel(
    0.0); // The pixel value converted to integers to populate the output image
  int out_depth =
    CV_8U; // The depth of output image. The default is unsigned 8-bit [0 255]
  ...
  if (argc < 4) { // Given an image, compute the mean pixel value out of the
                  // input image
    // Load the image
    ...
    // Compute the mean pixel of the input image, reducing to one value per
    // channel
    ...
    for (int c = 0; c < num_channels; ++c) {
      ...
      if ((pixel_m[c] < 0.0) ||
          (std::numeric_limits<_depth_type(CV_8U)>::max() < pixel_m[c])) {
        std::cerr << "Pixel value must be specified in the scale [0.0 255.0] "
                     "or in the unit scale [0.0 1.0]";
        return 0;
      }
      // Assume the unit scale if all the channel values are within the range
      // [0.0 1.0]
      if (1.0 < pixel_m[c])
        is_unit_scaled = false;
    }
  ...
  mean_image =
    cv::Mat(height, width, CV_MAKETYPE(out_depth, num_channels), pixel);
