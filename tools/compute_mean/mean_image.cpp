...
#include "lbann/data_ingestion/readers/opencv_extensions.hpp"
#include <cmath>
#include <iostream>
#include <mpi.h>
#include <set>
...
cv::Mat reduce_mean_images(const cv::Mat mean_image, const mpi_states& ms)
{
  ...
  if (!lbann::check_if_cv_Mat_is_float_type(mean_image)) {
    ...
    const double alpha =
      lbann::get_depth_normalizing_factor(mean_image.depth());
  ...
  const unsigned int num_values =
    local_data.rows * local_data.cols * local_data.channels();
  if (num_values == 0u) {
    return cv::Mat();
  }
  ...
  int mc = MPI_SUCCESS;
  mc = MPI_Allreduce(const_cast<void*>(_ConstVoidP(local_data.data)),
                     _VoidP(global_sum.data),
                     num_values,
                     ((local_data.depth() == CV_32F) ? MPI_FLOAT : MPI_DOUBLE),
                     MPI_SUM,
                     ms.get_comm());
  ms.check_mpi(mc);
  return global_sum;
 ...
 */
cv::Mat
divide_sum_image(const cv::Mat& global_sum, const int num_ranks, const int type)
{
  if (global_sum.empty() || (num_ranks <= 0)) {
    return cv::Mat();
  }

  static const std::set<int> depths =
  ...
  cv::Mat global_mean;
  const double alpha =
    lbann::get_depth_denormalizing_factor(CV_MAT_DEPTH(type)) / num_ranks;
                      ...
                      const std::string& out_dir)
{
  ...
  if (dynamic_cast<const lbann::cv_mean_extractor*>(
        pp.get_transform(mean_extractor_idx)) == nullptr) {
    return false;
  }

  cv::Mat local_mean;
  local_mean = dynamic_cast<const lbann::cv_mean_extractor*>(
                 pp.get_transform(mean_extractor_idx))
                 ->extract<void>();
  ...
  if (ms.is_root()) { // write a visible image of mean data
    cv::Mat global_mean =
      divide_sum_image(global_sum, ms.get_effective_num_ranks(), CV_8UC3);
    cv::imwrite(out_dir + "mean.png", global_mean);
  }

  if (ms.is_root()) { // write binary image data (in the numeric format as is.
                      // e.g., floating point)
    cv::Mat global_mean = divide_sum_image(global_sum,
                                           ms.get_effective_num_ranks(),
                                           local_mean.type());
    ...
    const size_t sz = global_mean.rows * global_mean.cols *
                      global_mean.channels() *
                      CV_ELEM_SIZE(global_mean.depth());

    // Either write into a pfm format image or a binary file (no handling of
    // endianess)
