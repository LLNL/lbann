...
fOrg=`grep OpenBLAS.cmake ${PATCH_DIR}/ChangedFiles.EL_OpenBLAS.txt 2> /dev/null`

OpenBLAS_TAG=`grep GIT_TAG ${fOrg} 2> /dev/null | grep -v OPENBLAS_TAG | awk '{if (NF==2) print $2; else print "0";}' | sed -e 's/\"//g'`

if [ "${OpenBLAS_TAG}" = "" ] && [ "${fOrg}" != "" ] && [ -f ${fOrg} ] ; then
