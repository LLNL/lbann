...
void get_sample_ids(const string& schema_str,
                    const conduit::Node& nd,
...
void get_all_sample_ids(const string& filelist,
...
bool is_leaf(const conduit::Node& nd) { return !nd.number_of_children(); }
...
void write_yaml_header(ofstream& out)
{
  out
    << "#You must edit this file prior to use as follows.\n"
       "#\n"
       "#  Uncomment the 'pack' entries for whichever data fields you wish to "
       "use.\n"
       "#  pack's value should be one of the following: datum, label, "
       "response.\n"
       "#  If you do not wish to use a data field, simply leave 'pack' "
       "commented out\n"
       "#\n"
       "#  For normalization, uncomment and add appropriate values to the "
       "'scale' \n"
       "#  and 'bias' entries.  For images, the 'scale' and 'bias' entries "
       "should be\n"
       "#  lists with one entry per channel. You should also add the following "
       "\n"
       "#  entries for images: dims, channels. Here is an example of the "
       "metadata\n"
       "#  entries for images:\n"
       "#    dims: [300,300]\n"
       "#    channels: 2\n"
       ...
       "#  e.g, when you have multiple 'pack: response' entries. The values in "
       "the \n"
       "#  ordering entries need not be unique and are relative\n"
       "#\n";
}

void write_yaml_file(ofstream& out,
                     const conduit::Node* nd,
                     int indent = 0,
                     int ordering = 10)
{
...
void write_exclusive_sample_list(ofstream& out,
                                 const map<string, vector<string>>& sample_ids,
...
void write_inclusive_sample_list(ofstream& out,
                                 const map<string, vector<string>>& sample_ids,
                                 const string& base_dir)
{
...
int main(int argc, char** argv)
{
  ...
  map<string, vector<string>> sample_ids; // maps: filename -> list of sample
                                          // IDs
