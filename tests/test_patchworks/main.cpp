...
bool test_patch(const int argc, char *argv[])
{
    ...
    std::string filename = argv[1];
    if (argc > 2) patch_size = std::atoi(argv[2]);
    if (argc > 3) gap = std::atoi(argv[3]);
    if (argc > 4) jitter = std::atoi(argv[4]);
    if (argc > 5) mode_centering = std::atoi(argv[5]);
    if (argc > 6) mode_chromaberr = std::atoi(argv[6]);

    if (patch_size == 0u) return false;
