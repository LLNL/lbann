...
class image {
  ...
  public:
    image(void) : m_window_title(""), m_filename("") { detect_screen_resolution(); }
    image(const std::string fname);
    virtual ~image(void);

    /// Check if an image data exists
    virtual bool empty(void) const { return (m_img.data == NULL); }
    /// Free the space used by image data
    virtual void release(void);
    /// Return the filename of this image
    virtual std::string get_filename(void) const { return m_filename; }
    /// Read an image file
    ...
    virtual int get_width(void) const { return m_img.cols; }
    /// Return the height (number of rows) of the image
    virtual int get_height(void) const { return m_img.rows; }

    /// Return the number of channels of the images
    virtual int get_num_channels(void) const { return m_img.channels(); }
    /// Return the pixel depth of the image in OpenCV term
    virtual int get_depth(void) const { return m_img.depth(); }

    /// Returns the access to the image data (OpenCV Mat type)
    virtual cv::Mat& get_image(void) { return m_img; }
    ...
    virtual bool write(const std::string outFileName) const { return write(outFileName, m_img); }
};
