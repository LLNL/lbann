...
def test_compiler_build_script(cluster, dirname):
    ...
    common_cmd = '%s/scripts/build_lbann.sh -d -l %s --test --clean-build -j $(($(nproc)+2)) -e scripts/common_spack_packages/ci_spack_packages.sh -- +deterministic +vision +numpy' % (dirname, ENV_NAME)
