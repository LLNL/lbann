...
def test_compiler_build_script(cluster, dirname):
    ...
    common_cmd = '%s/scripts/build_lbann.sh -r -l %s --clean-build -j $(($(nproc)+2)) -e %s/scripts/common_spack_packages/ci_spack_packages.sh -- +deterministic +vision +numpy +unit_tests' % (dirname, ENV_NAME, dirname)
