...
def construct_model(lbann):
    ...
    # Layer graph
    input_ = lbann.Slice(lbann.Input(data_field='samples'), slice_points=lbann.util.str_list([0, 1, 1+np.prod(config.input_shape)]))
    ...
    log_probs = lbann.LogSoftmax(lbann.FullyConnected(x, num_neurons=config.num_classes, has_bias=False))
    label_onehot = lbann.OneHot(labels, size=config.num_classes)
    loss = lbann.Negative(lbann.Reduction(lbann.Multiply(log_probs, label_onehot), mode="sum"))
    ...
    def func(cluster, dirname):
        ...
        with open(experiment_output['stdout_log_file']) as f:
            for line in f:
                ...
                match = re.search('test mini-batch time statistics : ([0-9.]+)s mean', line)
                if match:
                    mini_batch_time = float(match.group(1))

        # Check if testing accuracy is within expected range
        assert (expected_test_loss_range[0]
                < test_loss
                < expected_test_loss_range[1]), \
