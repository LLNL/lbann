...
def augment_test_func(test_func):
    ...
    def func(cluster, dirname, weekly):
        ...
        assert ((test_pc > targets['expected_test_range'][0]
                 and test_pc < targets['expected_test_range'][1])), \
                f"test reconstruction error {test_pc:.3f} is outside expected range " + \
                f"[{targets['expected_test_range'][0]:.3f},{targets['expected_test_range'][1]:.3f}]"
