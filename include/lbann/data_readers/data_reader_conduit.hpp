...
#ifndef _DATA_READER_CONDUIT_HPP_
#define _DATA_READER_CONDUIT_HPP_
 ...
 */
class data_reader_conduit : public generic_data_reader {
 public:
   ...
   * - JAG_TimeSeries: simulation input parameters
   * - Undefined: the default
   */
  enum variable_t {Undefined=0, JAG_Image, JAG_Scalar, JAG_Input, JAG_TimeSeries};
  ...
  data_reader_conduit(bool shuffle = true);
  data_reader_conduit(const std::shared_ptr<cv_process>& pp, bool shuffle = true);
  data_reader_conduit(const data_reader_conduit&);
  data_reader_conduit(const data_reader_conduit&, const std::vector<int>& ds_sample_move_list);
  data_reader_conduit& operator=(const data_reader_conduit&);
  ~data_reader_conduit() override;
  data_reader_conduit* copy() const override { return new data_reader_conduit(*this); }

  void setup(int num_io_threads, std::shared_ptr<thread_pool> io_thread_pool) override;

  std::string get_type() const override {
    return "data_reader_conduit";
  }
  ...
  void set_leading_reader(data_reader_conduit* r);
  /// Get the leader of local data reader group
  data_reader_conduit* get_leading_reader();
 ...
 protected:
  ...
  virtual void check_image_data() { LBANN_ERROR("shouldn't be here"); }
  /// Make sure that the keys to choose inputs are valid
  virtual void check_input_keys() { LBANN_ERROR("shouldn't be here"); }
  ...
  virtual void copy_members(const data_reader_conduit& rhs, const std::vector<int>& ds_sample_move_list = std::vector<int>());
  ...
  std::vector<El::Int> get_slice_points(const std::vector< std::vector<data_reader_conduit::variable_t> >& var) const;
 ...
 protected:
   ...
   */
  data_reader_conduit* m_leading_reader;
 ...
 */
template<typename S>
inline size_t data_reader_conduit::add_val(const std::string key, const conduit::Node& n, std::vector<S>& vals) {
  size_t cnt = 0u;
  ...
  switch (n.dtype().id()) {
    ...
    default:
      std::string err = std::string("data_reader_conduit::add_val() : invalid dtype (")
...
#endif // _DATA_READER_CONDUIT_HPP_
