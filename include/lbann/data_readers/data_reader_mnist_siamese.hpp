...
class data_reader_mnist_siamese : public data_reader_multi_images {
 ...
 protected:
   ...
   */
  virtual bool fetch_datum(CPUMat& X, std::pair<int, int> data_id, int mb_idx, thread_pool& io_thread_pool);
  /**
   ...
   * the given matrix Y.
   */
  virtual bool fetch_label(CPUMat& Y, std::pair<int, int> data_id, int mb_idx, thread_pool& io_thread_pool);
