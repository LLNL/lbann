...
template <typename T>
void lbann_comm::world_broadcast(int root, T& val) {
  broadcast(root, val, get_world_comm());
}
/// Inter-trainer broadcast of a scalar.
template <typename T>
void lbann_comm::intertrainer_broadcast(int root, T& val) {
  broadcast(root, val, get_intertrainer_comm());
}
/// Within-trainer broadcast of a scalar.
template <typename T>
void lbann_comm::trainer_broadcast(int root, T& val) {
...
template <typename T>
void lbann_comm::intertrainer_broadcast(const int root, T* data, const int count) {
 ...
 */
template <typename T>
size_t lbann_comm::resize(const int root, std::vector<T> &data, const El::mpi::Comm& c) {
 ...
 */
template <typename T>
void lbann_comm::broadcast(const int root, std::vector<T> &data, const El::mpi::Comm& c) {
...
template <typename T>
void lbann_comm::all_gather(const T* src, int src_count, T* rcv, int rcv_count, const El::mpi::Comm& c) {
 ...
 */
template <typename T>
void lbann_comm::all_gather(std::vector<T> &src, std::vector<T> &rcs, std::vector<int> &rcv_counts, std::vector<int> &rcv_disp, const El::mpi::Comm& c) {
  if (src.size() == 0) {
 ...
 */
template <typename T>
void lbann_comm::trainer_all_gather(std::vector<T> &src, std::vector<T> &rcs, std::vector<int> &rcv_counts, std::vector<int> &rcv_disp) {
 ...
 */
template <typename T>
void lbann_comm::all_gather(T &src, std::vector<T> &data, const El::mpi::Comm& c) {
...
template <typename T>
void lbann_comm::trainer_gather(T snd, int root) {
  gather(snd, root, trainer_comm);
}
/** Within-trainer scalar gather (for root processes). */
template <typename T>
void lbann_comm::trainer_gather(T snd, T* rcv) {
...
template <typename T>
void lbann_comm::trainer_gather(T* snd, int count, T* rcv) {
...
template <typename T>
void lbann_comm::trainer_gatherv(T* snd, int count, T* rcv, int* rcv_counts,
...
template <typename T>
void lbann_comm::intertrainer_gather(T snd, int root) {
...
template <typename T>
T lbann_comm::scatter(int root, const El::mpi::Comm& c) {
  T val = {};
...
template <typename T>
T lbann_comm::scatter(T *snd, const El::mpi::Comm& c) {
...
template <typename T>
T lbann_comm::intertrainer_reduce(T snd, El::mpi::Op op) {
...
template <typename T>
T lbann_comm::trainer_reduce(T snd, El::mpi::Op op) {
...
template <typename T, El::Device D>
void lbann_comm::reduce(T *snd, int count, int root, const El::mpi::Comm& c, El::SyncInfo<D> const& syncInfo) {
  reduce(snd, count, root, c, El::mpi::SUM, syncInfo);
}

template <typename T>
void lbann_comm::reduce(T *snd, int count, int root, const El::mpi::Comm& c, El::mpi::Op op) {
  reduce(snd, count, root, c, op, El::SyncInfo<El::Device::CPU>{});
}
template <typename T, El::Device D>
void lbann_comm::reduce(T *snd, int count, int root, const El::mpi::Comm& c, El::mpi::Op op, El::SyncInfo<D> const& syncInfo) {
  bytes_sent += sizeof(T) * count;
  El::mpi::Reduce(snd, (T*) NULL, count, op, root, c, syncInfo);
}
/** Scalar-array reduce (for root processes). */
template <typename T, El::Device D>
void lbann_comm::reduce(T *snd, int count, T *rcv, const El::mpi::Comm& c, El::SyncInfo<D> const& syncInfo) {
...
template <typename T>
void lbann_comm::reduce(T *snd, int count, T *rcv, const El::mpi::Comm& c, El::mpi::Op op) {
    reduce(snd, count, rcv, c, op, El::SyncInfo<El::Device::CPU>{});
}
template <typename T, El::Device D>
void lbann_comm::reduce(T *snd, int count, T *rcv, const El::mpi::Comm& c, El::mpi::Op op, El::SyncInfo<D> const& syncInfo) {
...
template <typename T>
T lbann_comm::trainer_allreduce(T snd, El::mpi::Op op) {
...
template <typename T>
void lbann_comm::allreduce(T *snd, int count, T *rcv, const El::mpi::Comm& c, El::mpi::Op op) {
  auto const size_c = El::mpi::Size(c);
...
template <typename T>
void lbann_comm::allreduce(T *data, int count, const El::mpi::Comm& c, El::mpi::Op op) {
  auto const size_c = El::mpi::Size(c);
  bytes_sent += count * sizeof(T);
#ifdef LBANN_HAS_ALUMINUM
#ifdef LBANN_ALUMINUM_MPI_PASSTHROUGH
 ...
 */
template <typename T>
void lbann_comm::nb_allreduce(T *data, int count, const El::mpi::Comm& c, Al::request& req,
                  El::mpi::Op op) {
#ifdef LBANN_HAS_ALUMINUM
  bytes_sent += count * sizeof(T);
...
template <typename T>
void lbann_comm::wait(El::mpi::Request<T>& req) {
  El::mpi::Wait(req);
}
...
template <typename T, El::Device D>
void lbann_comm::send(const T *data, int count, int trainer, int rank, El::SyncInfo<D> const& syncInfo) {
  bytes_sent += sizeof(T) * count;
  El::mpi::Send(data, count, get_world_rank(trainer, rank), get_world_comm(), syncInfo);
}
template <typename T, El::Device D>
void lbann_comm::send(const T *data, int count, int trainer, El::SyncInfo<D> const& syncInfo) {
  send(data, count, trainer, rank_in_trainer, syncInfo);
}

/** Corresponding non-blocking sends. */
template <typename T>
void lbann_comm::nb_send(const T *data, int count, int trainer, int rank,
             El::mpi::Request<T>& req) {
  bytes_sent += sizeof(T) * count;
  El::mpi::ISend(data, count, get_world_rank(trainer, rank), get_world_comm(), req);
}
template <typename T>
void lbann_comm::nb_tagged_send(const T *data, int count, int rank, int tag,
             El::mpi::Request<T>& req, const El::mpi::Comm& c) {
  bytes_sent += sizeof(T) * count;
  El::mpi::TaggedISend(data, count, rank, tag, c, req);
}
template <typename T> void lbann_comm::nb_send(const T *data, int count, int trainer,
                                   El::mpi::Request<T>& req) {
  nb_send(data, count, trainer, rank_in_trainer, req);
}

/** Corresponding receive to send. */
template <typename T> void lbann_comm::recv(T *data, int count, int trainer, int rank) {
...
template <typename T, El::Device D>
void lbann_comm::recv(T *data, int count, int trainer, int rank, El::SyncInfo<D> const& syncInfo) {
  El::mpi::Recv(data, count, get_world_rank(trainer, rank), get_world_comm(), syncInfo);
  bytes_received += sizeof(T) * count;
}
template <typename T, El::Device D>
void lbann_comm::recv(T *data, int count, int trainer, El::SyncInfo<D> const& syncInfo) {
...
template <typename T> void lbann_comm::nb_recv(T *data, int count, int trainer, int rank,
                                   El::mpi::Request<T>& req) {
  El::mpi::IRecv(data, count, get_world_rank(trainer, rank), get_world_comm(),
             req);
  bytes_received += sizeof(T) * count;
}
template <typename T> void lbann_comm::nb_tagged_recv(
...
template <typename T> void lbann_comm::nb_recv(T *data, int count, int trainer,
                                   El::mpi::Request<T>& req) {
  nb_recv(data, count, trainer, rank_in_trainer, req);
}
template <typename T> void lbann_comm::nb_recv(T *data, int count, El::mpi::Request<T>& req) {
...
template <typename T, El::Device D>
void lbann_comm::sendrecv(const T *snd, int send_count, int send_trainer, int send_rank,
...
template <typename T, El::Device D>
void lbann_comm::sendrecv(const T *snd, int send_count, int send_trainer,
...
template <typename T>
int lbann_comm::get_count(int trainer) {
