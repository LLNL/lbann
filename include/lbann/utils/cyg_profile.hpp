...
static c_hash_node* c_hash_lookup(void* func)
  __attribute__((no_instrument_function));
...
static unsigned long long djb2_hash(void*, int table_size)
  __attribute__((no_instrument_function));
...
static void c_hash_insert(void* func)
{
  ...
  if (c_hash_info.dli_sname != NULL) {
    int success = 0;
    char* demangled_name =
      abi::__cxa_demangle(c_hash_info.dli_sname, nullptr, nullptr, nullptr);
      ...
      for (long i = 0; i < h->size; i++) {
        ...
        if (h->data[idx].mark < h->cur_mark) {
          ...
          if (c_hash_fp_full_stack_trace_metadata != NULL) {
            fprintf(c_hash_fp_full_stack_trace_metadata,
                    "%hd %s\n",
                    c_hash_func_id,
                    demangled_name);
            fwrite(&c_hash_func_id,
                   sizeof(short),
                   1,
                   c_hash_fp_full_stack_trace);
