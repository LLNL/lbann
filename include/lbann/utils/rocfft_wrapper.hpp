...
////////////////////////////////////////////////////////////////////////////////
#ifndef LBANN_UTILS_ROCFFT_WRAPPER_HPP_
#define LBANN_UTILS_ROCFFT_WRAPPER_HPP_
...
#define LBANN_CHECK_ROCFFT(cmd)                                           \
  do {                                                                    \
    auto const lbann_check_rocfft_result_ = (cmd);                        \
    if (lbann_check_rocfft_result_ != ROCFFT_SUCCESS) {                   \
      LBANN_ERROR(                                                        \
        "rocFFT error!\n\n"                                               \
        "      cmd: " #cmd "\n"                                           \
        "   result: ",                                                    \
        lbann::rocfft::value_as_string(lbann_check_rocfft_result_), "\n"  \
        "  message: ",                                                    \
        lbann::rocfft::result_string(lbann_check_rocfft_result_),"\n\n"); \
    }                                                                     \
  } while (0)
...
std::string value_as_string(rocfftResult_t);
/** @brief The docstring for the given result. */
std::string result_string(rocfftResult_t);

template <typename T>
struct rocFFTTypeT;

template <> struct rocFFTTypeT<float> { using type = float; };
template <> struct rocFFTTypeT<double> { using type = double; };
template <>
struct rocFFTTypeT<El::Complex<float>> { using type = rocfftComplex; };
template <>
struct rocFFTTypeT<El::Complex<double>> { using type = rocfftDoubleComplex; };

template <typename T>
using rocFFTType = typename rocFFTTypeT<T>::type;

template <typename T>
auto AsCUFFTType(T* buffer)
{
  return reinterpret_cast<rocFFTType<T>*>(buffer);
}

/** @brief Alias around the C-compatible API */
template <typename InType, typename OutType>
struct rocFFTExecutor;

template <>
struct rocFFTExecutor<El::Complex<float>, El::Complex<float>>
{
  static constexpr auto transform_type = CUFFT_C2C;
  static void Execute(rocfftHandle plan,
...
};// struct rocFFTExecutor<Complex<float>, Complex<float>>

template <>
struct rocFFTExecutor<El::Complex<double>, El::Complex<double>>
{
...
};// struct rocFFTExecutor<Complex<double>, Complex<double>>

/** @brief Wrapper around rocFFT
 *
 ...
 *  rocFFT interface is simpler and better in many ways.
 */
...
public:
  ...
  using ExecutorType = rocFFTExecutor<InputType, OutputType>;
  using PlanType = rocfftHandle;
...
public:
  rocFFTWrapper() = default;
  ~rocFFTWrapper() = default;
  // Movable, not copyable.
  rocFFTWrapper(rocFFTWrapper&& other) noexcept = default;
  rocFFTWrapper(rocFFTWrapper const&) = delete;
...
private:
  void compute_common(OutputMatType& in, InputMatType& out, int dir) const
  {
    ...
    if (good_plan == cend(plans_))
      LBANN_ERROR("No valid rocFFT plan found.");

    // Setup the workspace
    ComplexBufferType workspace(good_plan->worksize_,
                                El::SyncInfoFromMatrix(out));
    LBANN_CHECK_CUFFT(
      rocfftSetWorkArea(good_plan->plan_, workspace.data()));
                    ...
                    std::vector<int> const& full_dims)
  {
    ...
    if (good_plan == cend(plans_))
    {
      ...
      // certain rocFFT doesn't change this data.
      std::vector<int> full_dims_mutable(full_dims);
      LBANN_CHECK_CUFFT(rocfftCreate(&plan));
      LBANN_CHECK_CUFFT(
        rocfftSetStream(plan, SyncInfoFromMatrix(out).Stream()));
      // We'll handle our own workspace
      LBANN_CHECK_CUFFT(rocfftSetAutoAllocation(plan, 0));
      ...
      if (feature_map_ndims > 3 || feature_map_ndims == 0)
        LBANN_ERROR("Only 1-, 2-, and 3-D FFTs are supported in rocFFT.");
      ...
      if (plan == 0)
        LBANN_ERROR("rocFFT plan construction failed "
                    "but rocFFT reported no errors.");
...
#endif // LBANN_UTILS_ROCFFT_WRAPPER_HPP_
