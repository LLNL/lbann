...
inline pooling_mode from_miopen(miopenPoolingMode_t m)
{
  switch(m)
  {
  case miopenPoolingMax: return pooling_mode::MAX;
  case miopenPoolingAverageInclusive: return pooling_mode::AVERAGE_COUNT_INCLUDE_PADDING;
  case miopenPoolingAverage: return pooling_mode::AVERAGE_COUNT_EXCLUDE_PADDING;
  default:
    LBANN_ERROR("Invalid pooling mode requested.");
  }
}
