...
using persist_type_iterator = enum_iterator<persist_type,
                                            persist_type::train,
...
public:
  ...
  callback_type get_cb_type() const { return ckpt_type; }

  void set_cb_type(callback_type type) { ckpt_type = type; }
  ...
  bool write_rank_distmat(persist_type type,
                          const char* name,
                          const El::AbstractDistMatrix<TensorDataType>& M);
  template <typename TensorDataType>
  bool read_rank_distmat(persist_type type,
                         const char* name,
                         El::AbstractDistMatrix<TensorDataType>& M);

  template <typename TensorDataType>
  bool write_distmat(persist_type type,
                     const char* name,
                     El::AbstractDistMatrix<TensorDataType>* M);
  template <typename TensorDataType>
  bool read_distmat(persist_type type,
                    const char* name,
                    El::AbstractDistMatrix<TensorDataType>* M);
...
public:
  NonexistentArchiveFile(std::string const& filename)
    : std::runtime_error(std::string("Archive file not found: ") + filename)
  {}
};
