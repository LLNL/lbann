...
class pooling_distconv_adapter : public data_type_distconv_adapter<TensorDataType> {
 public:
  ...
  void fp_compute(bool training = true); // training=true for max back-compatibility.
  void bp_compute();
  ...
  void fp_compute() override {
    if(this->using_gpus()) {
      ...
      if (this->distconv_enabled()) {
        ...
        if (counter % 10000 == 0) {
          std::cout << "** DC POOLING: " << counter
                    << " (this=" << this << ", mode=" << to_string(mode) << ") **"
                    << std::endl;
        }
    ...
    else {
      {
        ...
        const auto& mode
          = this->m_model->get_execution_context().get_execution_mode();
        std::cout << "** IM2COL POOLING: " << ++counter
                  << " (this=" << this << ", mode=" << to_string(mode) << ") **"
                  << std::endl;
      }
      fp_compute_im2col();
...
template <typename TensorDataType, data_layout Layout, El::Device Device>
void pooling_distconv_adapter<TensorDataType, Layout, Device>::
fp_compute(bool const training) {
  m_pooling->forward(El::To<TensorDataType>(1), this->get_prev_activations(),
                     El::To<TensorDataType>(0), this->get_activations(), training);
}
