...
class branch_layer : public transform {
 ...
 public:
  ...
  void add_child(const Layer *child) {
    ...
    #ifdef LBANN_DEBUG
      if(m_comm->am_world_master()) {
        std::cerr << "branch_layer: could not add child layer it is already in list of children" << "\n";
      }
    #endif
    }
  ...
  void remove_child(const Layer *child) {
    ...
    #ifdef LBANN_DEBUG
      if(m_comm->am_world_master()) {
        std::cerr << "branch_layer: could not remove child layer it isn't in list of children" << "\n";
      }
    #endif
    }
