...
args.convtype = util.ConvType.bwdfilt
...
def to_channels_last_matrix(dy: y_desc, result, MB_TILE: dace.compiletime, TILE_OFFSET):
    rdy = np.reshape(dy, (B, COUT, MB))
    reshaped_result = np.reshape(result, [B, MB_TILE, COUT])
    for n, c, j in dace.map[0:B, 0:COUT, 0:MB_TILE]:
        reshaped_result[n, j, c] = rdy[n, c, j + TILE_OFFSET]

def im2col(im, col, MB_TILE: dace.compiletime, TILE_OFFSET):
    for n, i, k in dace.map[0:B, 0:MB_TILE, 0:K]:
        o: dace.int64
        opq_res: dace.int64
        p: dace.int64
        q: dace.int64
        ...
        o, opq_res = divmod(i + TILE_OFFSET, HOUT*WOUT)
        ...
        col[i + n*MB_TILE, k] = elem


@dace.program
def explicit_gemm_bwdfilter(dw: w_desc, dx: x_desc, dy: y_desc, alpha: dtype, beta: dtype):
    # Allocations moved outside to avoid allocating multiple versions of the arrays
    channels_last = np.empty([B*MB_WSTILE, N], dtype=dtype)
    i2c_x = np.empty([B*MB_WSTILE, K], dtype=dtype)

    # M (=B * MB) is divided into multiple workspace tiles, summing over different
    # contributions of dy and x every time.
    # Scheme to support alpha/beta and reduce over multiple workspace tiles:
    # One tile (M_WSTILES = 1): alpha = alpha, beta = beta
    # More than one tile: alpha remains, first iteration keeps beta (in order to
    #                     preserve the values of dw). Subsequent iterations use
    #                     beta = 1 to summarize values

    # (1) Compute first tile
    to_channels_last_matrix(dy, channels_last, MB_WSTILE, 0)
    im2col(dx, i2c_x, MB_WSTILE, 0)
    dace.libraries.blas.Gemm(channels_last, i2c_x, dw, alpha, beta, trans_a=True, trans_b=False)

    # (2) Compute subsequent, evenly-divided tiles
    if M_WSTILES > 1:
        for T in range(1, M_WSTILES):
            to_channels_last_matrix(dy, channels_last, MB_WSTILE, T*MB_WSTILE)
            im2col(dx, i2c_x, MB_WSTILE, T*MB_WSTILE)
            dace.libraries.blas.Gemm(channels_last, i2c_x, dw, alpha, 1.0, trans_a=True, trans_b=False)

    # (3) Compute remainder tile
    if MB_WSTILE_REM != 0:
        to_channels_last_matrix(dy, channels_last, MB_WSTILE_REM, M_WSTILES*MB_WSTILE)
        im2col(dx, i2c_x, MB_WSTILE_REM, M_WSTILES*MB_WSTILE)
        dace.libraries.blas.Gemm(channels_last[:B*MB_WSTILE_REM], i2c_x[:B*MB_WSTILE_REM], dw, alpha, 1.0, trans_a=True, trans_b=False)
...
if __name__ == '__main__':
    ...
    sdfg = explicit_gemm_bwdfilter.to_sdfg()
    ...
    if storage == dace.StorageType.CPU_Heap and os.path.exists('out_dw.bin'):
        print('Files found, verifying result')
        x, dw, dy = util.conv_bwdfilt_inputs()
        csdfg(dw, x, dy, alpha=np.float32(1.0), beta=np.float32(0.0))
        util.verify_bwdfilt(dw)
