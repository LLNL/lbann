...
class GCN_Layer(Module):
    ...
    def forward(self, X, A):
        ...
        for i in X.shape[0]:
            X[i] = lbann.MatMul(X[i], self.W, name=self.name+'_message')
        out = X.get_mat()
...
class Dense_Graph_Layer(Module):
    ...
    def forward(self, X, A):
        ...
        messages = lbann.MatMul(A,X,name=self.name+'_message')

        ident = lbann.Mat_Mul(X, selt.W1, name=self.name+'_inner')
