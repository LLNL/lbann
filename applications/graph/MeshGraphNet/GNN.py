...
def input_data_splitter(input_layer,
  ...
  node_features = lbann.Reshape(lbann.Identity(sliced_input),
                                dims=[num_nodes, in_dim_node])
  edge_features = lbann.Reshape(lbann.Identity(sliced_input),
                                dims=[num_edges, in_dim_edge])
  ...
  out_features = lbann.Reshape(lbann.Identity(sliced_input),
                               dims=[num_nodes, out_dim])

  return node_features, edge_features, source_node_indices, \
    target_node_indices, out_features


def LBANN_GNN_Model(num_nodes, num_edges,
                    in_dim_node, in_dim_edge, out_dim,
  ...
  node_encoder  = MLP(in_dim=in_dim_node, out_dim=out_dim_node,
                      hidden_dim=hidden_dim_node, hidden_layers=hidden_layers_node,
                      norm_type=norm_type, name="graph_input_node_encoder")

  edge_encoder  = MLP(in_dim=in_dim_edge, out_dim=out_dim_edge,
                      hidden_dim=hidden_dim_edge, hidden_layers=hidden_layers_edge,
                      norm_type=norm_type, name="graph_input_edge_encoder")

  graph_processor = GraphProcessor(num_nodes=num_nodes,
                                   mp_iterations=mp_iterations,
                                   in_dim_node=out_dim_node, in_dim_edge=out_dim_edge,
  ...
  node_decoder = MLP(in_dim=out_dim_node, out_dim=out_dim,
                      hidden_dim=hidden_dim_decoder, hidden_layers=hidden_layers_decoder,
                      norm_type=None, name="graph_input_node_decoder")
  ...
  node_features, edge_features, source_node_indices, target_node_indices,\
     out_features = input_data_splitter(input_layer,
                                        num_nodes,
                                        num_edges,
                                        in_dim_node,
                                        in_dim_edge,
  ...
  model = lbann.Model(num_epochs, layers=layers, objective_function=loss,
                      callbacks=callbacks)
  return model
