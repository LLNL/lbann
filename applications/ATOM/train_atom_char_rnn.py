...
def construct_lc_launcher_args():
    ...
    parser.add_argument('--data-module-file', help="specifies the module that contains the logic for loading data")
    parser.add_argument('--data-config', help="path to a data config file that is used for the construction of the data reader")  #TODO(derek): just put all data relevant info into the config, condense condense condense 
    parser.add_argument('--time-limit', type=int, default=720, help="specified time limit in number of minutes")
    ...
    parser.add_argument('--hidden', type=int, default=768, help="size of the hidden layer")
    parser.add_argument('--lr', type=float, default=1e-3, help='optimizer learning rate to use for training')
    parser.add_argument('--num-layers', type=int, default=1, help='number of LSTM layers')
    parser.add_argument('--step-size', type=int, default=10, help='learning rate decay step size')
...
def construct_model(run_args):
    ...
    print("sequence length before layer graph is {}".format(sequence_length))
    ...
    fc = lbann.modules.FullyConnectedModule(size=num_embeddings, data_layout=data_layout)
    ...
    for i in range(sequence_length-1):
        ...
        pad_mask = lbann.NotEqual([idl[i],lbann.Constant(value=run_args.pad_index,num_neurons='1')],device='CPU')  #TODO: am adding a .config that will be used to read the value of pad index
...
def construct_merged_data_reader(run_args):
    """
    Construct Protobuf message for Python data reader.
...
if __name__ == '__main__':
    ...
    if run_args.chunked_data:
        #data_reader = construct_merged_data_reader(run_args)
        import os.path
        import lbann
        import google.protobuf.text_format as txtf    
    ...
    experiment_dir = os.path.join(work_dir, '{}_{}'.format(timestamp, run_args.job_name))
