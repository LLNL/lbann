...
void lbann::partitioned_minibatch_parallel_io::calculate_num_iterations_per_epoch(generic_data_reader *data_reader) {
  ...
  int num_parallel_readers_per_model = max(1, (data_reader->m_batch_stride / m_comm->get_num_models()) / max_mini_batch_size);
  ...
  if(/*m_comm->get_rank_in_model() == parallel_readers_with_extra_mini_batch && */per_model_partial_mini_batch_size > 0 || world_master_remainder_adjustment > 0) {
    data_reader->m_last_mini_batch_stride = (data_reader->m_last_mini_batch_threshold - data_reader->m_base_offset - data_reader->m_model_offset - last_mini_batch_offset) + m_comm->get_rank_in_model();
  }
