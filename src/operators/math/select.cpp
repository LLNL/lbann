...
struct SelectImpl
{
  ...
  DataT operator()(DataT const& predicate,
                   DataT const& iftrue,
  ...
  std::vector<LocalOutputTensorType> outputs) const
{
  ...
  if (!m_constant_if_false && !m_constant_if_true) {
    ...
    El::EntrywiseMap(
      inputs.front().data(),
      outputs.front().data(),
      std::function<DataT(DataT const&)>(SelectImplConstant(m_value,
                                                            m_epsilon,
                                                            m_value_if_true,
                                                            m_value_if_false)));
  }
...
#define PROTO(T) template class SelectOperator<T, El::Device::CPU>
