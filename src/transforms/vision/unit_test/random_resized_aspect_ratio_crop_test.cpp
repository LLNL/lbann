...
TEST_CASE("Testing random resized aspect ratio crop preprocessing", "[preproc]") {
  ...
  SECTION("matrix with one channel") {
    ...
    SECTION("resizing larger and cropping") {
      auto resize_cropper = lbann::transform::random_resized_aspect_ratio_crop(3, 3);
    ...
    SECTION("resizing smaller and cropping") {
      auto resize_cropper = lbann::transform::random_resized_aspect_ratio_crop(1, 1);
