...
typedef boost::property<
  boost::edge_name_t, std::string> edge_property;
...
void dump_model_graph::on_setup_end(model *m) {
  ...
  const auto get_tensor_dims_str =
    ...
    for(const Layer* layer : layers) {
      ...
      if(layer->get_num_parents()) {
        for(int i = 0; i < layer->get_num_parents(); i++) {
          ...
          ss << (i ? ", " : "") << get_tensor_dims_str(input_dims) << " (" << layer->get_parent_layer(i).get_name() << ")";
        }
      } else {
        ss << "no input";
      }
      ...
      if(layer->get_num_children()) {
        for(int i = 0; i < layer->get_num_children(); i++) {
          ...
          ss << (i ? ", " : "") << get_tensor_dims_str(output_dims) << " (" << layer->get_child_layer(i).get_name() << ")";
        }
      } else {
        ss << "no output";
      }
  ...
  const auto get_vertex_color =
      [](const std::string layer_type) {
        const size_t hash = (std::hash<std::string>()(layer_type)&0xFFFFFF) | 0x808080;
  ...
  for(unsigned int i = 0; i < layers.size(); i++) {
    ...
    g[v].shape = l->get_num_parents() == 0 || l->get_num_children() == 0 ? "ellipse" : "box";
    g[v].style = "filled";
    for(int i_parent = 0; i_parent < l->get_num_parents(); i_parent++)
      edges.emplace(l->get_parent_layers()[i_parent], l,
  ...
  for(const auto& edge : edges) {
    ...
    const auto i_to = std::distance(
        layers.begin(), std::find(layers.begin(), layers.end(), std::get<1>(edge)));
    boost::add_edge(vertexs[i_from], vertexs[i_to], edge_property(std::get<2>(edge)), g);
  }
