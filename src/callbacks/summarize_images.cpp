...
//FIXME: Should width of img_layer abd accuracy_layer activations be tested here?
  ...
  if (categorized_correctly.Height() != El::Int(1))
    LBANN_ERROR_STR("Tom was wrong about this matrix. Oops.");
  ...
  if (categorized_correctly.CrossRank() == categorized_correctly.Root()) {
    ...
    for (auto sample = decltype(num_samples){0}; sample < num_samples; ++sample) {
      ...
      if ((correctness_value != DataType(0))
        ...
        LBANN_ERROR_STR("Invalid data from ", m_cat_accuracy_layer->get_name(),
...
//FIXME: Correct params?
  return make_unique<summarize_images>(
    summarizer,
    nullptr,
//    build_strategy(params.selection_strategy()),
    params.image_layer(),
    params.input_layer(),
    params.interval());
}
