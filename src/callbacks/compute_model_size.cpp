...
compute_model_size::compute_model_size(std::string output_name,
  ...
  : callback_base(batch_interval), m_output_name(std::move(output_name))
{
  m_output = nullptr;
}

compute_model_size::compute_model_size() : compute_model_size("", 0) {}
...
void compute_model_size::compute_size(model& m)
{
  ...
  if (m.get_comm()->am_trainer_master()) {
    std::cout << "Trainer [ " << m.get_comm()->get_trainer_rank() << " ], Step "
...
{
  const auto& params =
    dynamic_cast<const lbann_data::Callback::CallbackComputeModelSize&>(
      proto_msg);
  return std::make_unique<compute_model_size>(params.output_name(),
