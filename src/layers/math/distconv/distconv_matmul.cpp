...
namespace distconv{
  ...
  ::forward(const tensor::Tensor<DataType, tensor::LocaleMPI, Allocator> &input_0,
                                                     ...
                                                     output_stride);
    {
      ...
      gpu_blas::GemmStridedBatched(
        ...
        input_1_mat.LockedBuffer(), input_1_width, input_1_stride,
        input_0_mat.LockedBuffer(), input_0_width, input_0_stride,
        El::TypeTraits<DataType>::Zero(),
        output_mat.Buffer(), output_width, output_stride,
        num_matrices,
        multisync);
    }
    return 0;
  ...
  ::backward(const tensor::Tensor<DataType, tensor::LocaleMPI, Allocator> &input_0,
    ...
    if (input_0.get_local_size() == 0 || input_1.get_local_size() == 0 ||
        ...
        input_0_grad.get_local_size() == 0 || input_0_grad.get_local_size() == 0){
      return 0;  // no op for emptry inputs
    }
    ...
    El::Matrix<DataType, El::Device::GPU> output_grad_mat(output_stride,
                                                          num_matrices,
                                                          output_grad.get_buffer(),
                                                          output_stride);

    El::Matrix<DataType, El::Device::GPU> input_0_grad_mat(input_0_stride,
                                                            num_matrices,
                                                            input_0_grad.get_buffer(),
                                                            input_0_stride);

    El::Matrix<DataType, El::Device::GPU> input_1_grad_mat(input_1_stride,
                                                           num_matrices,
                                                           ...
                                                           input_1_stride);
    {
      ...
      if (transpose_0){
        gpu_blas::GemmStridedBatched(
          ...
          transpose_1 ? TransposeMode::TRANSPOSE : TransposeMode::NORMAL,
          input_0_width, input_0_height, output_width,
          El::TypeTraits<DataType>::One(),
          output_grad_mat.LockedBuffer(), output_width, output_stride,
          input_1_mat.LockedBuffer(), input_1_width, input_1_stride,
          El::TypeTraits<DataType>::Zero(),
          input_0_grad_mat.Buffer(), input_0_width, input_0_stride,
          num_matrices,
          multisync);
      }else{
        gpu_blas::GemmStridedBatched(
          transpose_1 ? TransposeMode::NORMAL : TransposeMode::TRANSPOSE,
    ...
    }
    {
      ...
      if (transpose_1){
        gpu_blas::GemmStridedBatched(
          transpose_0 ? TransposeMode::TRANSPOSE : TransposeMode::NORMAL,
          TransposeMode::TRANSPOSE,
          input_1_width, input_1_height, output_height,
          El::TypeTraits<DataType>::One(),
          input_0_mat.LockedBuffer(), input_0_width, input_0_stride,
          output_grad_mat.LockedBuffer(), output_width, output_stride,
          El::TypeTraits<DataType>::Zero(),
          input_1_grad_mat.Buffer(), input_1_width, input_1_stride,
          num_matrices,
          multisync);
      }else{
        gpu_blas::GemmStridedBatched(
          TransposeMode::NORMAL,
          transpose_0 ? TransposeMode::NORMAL : TransposeMode::TRANSPOSE,
          input_1_width, input_1_height, output_height,
          El::TypeTraits<DataType>::One(),
          output_grad_mat.LockedBuffer(), output_width, output_stride,
          input_0_mat.LockedBuffer(), input_0_width, input_0_stride,
