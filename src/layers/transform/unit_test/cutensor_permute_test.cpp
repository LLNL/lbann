          ...
          "[permute][layer][cutensor]")
{
  ...
  SECTION("no permutation (0, 1, 2, 3)")
  {
    ...
    cuTENSOR_PermuteImpl permuter{RowMajorPerm{lbann_perm}};
    permuter.set_dims(RowMajor(vec_convert<int64_t>(lbann_dims)));

    auto const input_dims = permuter.input_dims();
    REQUIRE(input_dims.get() == std::vector<int64_t>{6, 5, 4, 3});

    auto const output_dims = permuter.output_dims();
    REQUIRE(output_dims.get() == std::vector<int64_t>{6, 5, 4, 3});

    auto const input_strides = permuter.input_strides();
    REQUIRE(input_strides.get() == std::vector<int64_t>{1, 6, 30, 120});

    auto const output_strides = permuter.output_strides();
    REQUIRE(output_strides.get() == std::vector<int64_t>{1, 6, 30, 120});

    auto const in_modes = permuter.input_modes();
    REQUIRE(in_modes == std::vector<int>{'a', 'b', 'c', 'd', 'e'});

    auto const out_modes = permuter.output_modes();
    REQUIRE(out_modes == std::vector<int>{'a', 'b', 'c', 'd', 'e'});
  }
  ...
  SECTION("permutation (0, 2, 1, 3)")
  {
    ...
    auto const output_dims = permuter.output_dims();
    REQUIRE(output_dims.get() == std::vector<int64_t>{6, 4, 5, 3});

    auto const input_strides = permuter.input_strides();
    REQUIRE(input_strides.get() == std::vector<int64_t>{1, 6, 30, 120});

    auto const output_strides = permuter.output_strides();
    REQUIRE(output_strides.get() == std::vector<int64_t>{1, 6, 24, 120});

    auto const in_modes = permuter.input_modes();
    REQUIRE(in_modes == std::vector<int>{'a', 'b', 'c', 'd', 'e'});

    auto const out_modes = permuter.output_modes();
    REQUIRE(out_modes == std::vector<int>{'a', 'c', 'b', 'd', 'e'});
  }
  ...
  SECTION("permutation (1, 2, 3, 0)")
  {
    ...
    auto const output_dims = permuter.output_dims();
    REQUIRE(output_dims.get() == std::vector<int64_t>{3, 6, 5, 4});

    auto const input_strides = permuter.input_strides();
    REQUIRE(input_strides.get() == std::vector<int64_t>{1, 6, 30, 120});

    auto const output_strides = permuter.output_strides();
    REQUIRE(output_strides.get() == std::vector<int64_t>{1, 3, 18, 90});

    auto const in_modes = permuter.input_modes();
    REQUIRE(in_modes == std::vector<int>{'a', 'b', 'c', 'd', 'e'});

    auto const out_modes = permuter.output_modes();
    REQUIRE(out_modes == std::vector<int>{'d', 'a', 'b', 'c', 'e'});
  }
  ...
  SECTION("permutation (1, 0, 3, 2)")
  {
    ...
    auto const output_dims = permuter.output_dims();
    REQUIRE(output_dims.get() == std::vector<int64_t>{5, 6, 3, 4});

    auto const input_strides = permuter.input_strides();
    REQUIRE(input_strides.get() == std::vector<int64_t>{1, 6, 30, 120});

    auto const output_strides = permuter.output_strides();
    REQUIRE(output_strides.get() == std::vector<int64_t>{1, 5, 30, 90});

    auto const in_modes = permuter.input_modes();
    REQUIRE(in_modes == std::vector<int>{'a', 'b', 'c', 'd', 'e'});

    auto const out_modes = permuter.output_modes();
    REQUIRE(out_modes == std::vector<int>{'b', 'a', 'd', 'c', 'e'});
  }
  ...
  SECTION("permutation (2, 3, 1, 0)")
  {
    ...
    auto const output_dims = permuter.output_dims();
    REQUIRE(output_dims.get() == std::vector<int64_t>{3, 4, 6, 5});

    auto const input_strides = permuter.input_strides();
    REQUIRE(input_strides.get() == std::vector<int64_t>{1, 6, 30, 120});

    auto const output_strides = permuter.output_strides();
    REQUIRE(output_strides.get() == std::vector<int64_t>{1, 3, 12, 72});

    auto const in_modes = permuter.input_modes();
    REQUIRE(in_modes == std::vector<int>{'a', 'b', 'c', 'd', 'e'});

    auto const out_modes = permuter.output_modes();
    REQUIRE(out_modes == std::vector<int>{'d', 'c', 'a', 'b', 'e'});
  }
}
...
TEST_CASE("iota_fill", "[permute][layer][gpu]")
{
  std::vector<int> const lbann_dims = {3, 4, 5, 6};
...
TEST_CASE("Tensor permutation", "[permute][layer][gpu]")
{
  std::vector<int> const lbann_dims = {3, 4, 5, 6};
  // strides = { 120, 30, 6, 1 }
  // perm = { 0, 2, 1, 3 }
