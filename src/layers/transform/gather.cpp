...
void gather_layer<TensorDataType, Layout, Device>::fp_compute() {
  ...
  const bool is_axis_0 = (input_dims.size()>1 && this->m_gather_axis == 0);  
  ...
  const size_t num_rows = (input_dims.size()>1) ? output_dims[0] : 1;
  
  const El::Int bounds = is_axis_0 ? input_dims[0]: values_size;
  ...
  for (size_t j=0; j<local_mini_batch_size; ++j) {
    for (size_t k = 0; k < num_rows; ++k){
      for (size_t i=0; i<output_size; ++i) {
        ...
        if (0<=ind && ind<bounds) {
          if(is_axis_0){
            local_output(k*output_size+i,j) = local_values(ind*values_size+i,j);
          }else{
...
void gather_layer<TensorDataType, Layout, Device>::bp_compute() {
  ...
  const El::Int bounds = is_axis_0? this->get_output_dims()[0]: values_size;
  ...
  for (size_t j=0; j<local_mini_batch_size; ++j) {
    for (size_t k = 0; k < num_rows; ++k){
      for (size_t i=0; i<output_size; ++i) {
        ...
        if (0<=ind && ind<bounds) {
          if(is_axis_0){
            local_values_grad(ind*values_size+i,j) += local_output_grad(k*output_size+i,j);
          }else{
