...
__global__ void fp_gpu_3d_kernel(El::Int input_dim0,
                                 El::Int input_dim1,
                                 El::Int input_dim2,
                                 El::Int /*output_dim0*/,
                                 El::Int output_dim1,
                                 El::Int output_dim2,
                                 El::Int local_output_height,
                                 El::Int local_output_width,
                                 const TensorDataType* __restrict__ input,
                                 El::Int input_ldim,
                                 TensorDataType* __restrict__ local_output,
                                 El::Int local_output_ldim,
                                 El::Int output_col_shift,
...
template <typename TensorDataType>
__global__ void
bp_gpu_3d_kernel(El::Int input_dim0,
                 El::Int input_dim1,
                 El::Int input_dim2,
                 El::Int /*output_dim0*/,
                 El::Int output_dim1,
                 ...
                 El::Int gradient_wrt_input_ldim)
{
  ...
  for (El::Int pos = gidx; pos < local_output_size; pos += num_threads) {
    ...
    const auto& output_pos0 = output_index / (output_dim1 * output_dim2);
    const auto& dy =
      local_gradient_wrt_output[local_row +
                                local_col * local_gradient_wrt_output_ldim];
    ...
    const auto& input_index = (input_pos0 * input_dim1 * input_dim2 +
                               input_pos1 * input_dim2 + input_pos2);
    auto& dx =
      gradient_wrt_input[input_index + local_col * gradient_wrt_input_ldim];
  ...
  El::AbstractDistMatrix<TensorDataType>& output)
{
  ...
  if (!local_output.IsEmpty()) {
    ...
    hydrogen::gpu::LaunchKernel(fp_gpu_3d_kernel<TensorDataType>,
                                grid_size,
                                block_size,
                                0,
                                multisync,
  ...
  El::AbstractMatrix<TensorDataType>& gradient_wrt_input)
{
  ...
  if (!local_gradient_wrt_output.IsEmpty()) {
    ...
    const auto& grid_size =
      (local_height * local_width + block_size - 1) / block_size;
    hydrogen::gpu::LaunchKernel(bp_gpu_3d_kernel<TensorDataType>,
                                grid_size,
                                block_size,
                                0,
                                multisync,
