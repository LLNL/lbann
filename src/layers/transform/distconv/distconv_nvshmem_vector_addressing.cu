...
atomic_add(float* __restrict__ address, const float val, const int pe)
{
  ...
  do {
    assumed = old;
    old = nvshmem_int_atomic_compare_swap(
      address_as_int,
      assumed,
...
atomic_add(double* __restrict__ address, const double val, const int pe)
{
  ...
  do {
    assumed = old;
    old = nvshmem_longlong_atomic_compare_swap(
      address_as_ll,
      assumed,
...
__device__ __forceinline__ float floor(const float& x) { return floorf(x); }

__device__ __forceinline__ double floor(const double& x) { return floor(x); }
...
__global__ void
Gather_NVSHMEM_Kernel(const DataType* __restrict__ shared_buffer,
                      ...
                      const int pe_stride)
{
  ...
  for (size_t mb_i = gidy; mb_i < mini_batch_size; mb_i += nthreadsy) {
    ...
    for (size_t row = gidx; row < num_local_output_rows * warp_size;
         row += nthreadsx) {
      const auto ind =
        static_cast<El::Int>(floor(indices[ind_offset + row / warp_size]));
      if (ind >= 0) {
        ...
        nvshmemx_getmem_nbi_warp(
          &output[mb_offset + (row / warp_size) * num_local_cols],
 ...
 */
template <typename DataType>
__global__ void Scatter_NVSHMEM_Kernel(const DataType* __restrict__ values,
                                       const DataType* __restrict__ indices,
                                       ...
                                       const int pe_stride)
{
  ...
  for (size_t mb_i = gidz; mb_i < mini_batch_size; mb_i += nthreadsz) {
    ...
    for (size_t row = gidy; row < num_local_values_rows; row += nthreadsy) {
      // Figure out which rank to send the vector
      const auto ind =
        static_cast<El::Int>(floor(indices[indices_offset + row]));
      if (ind >= 0) {
        ...
        for (size_t i = gidx; i < num_cols; i += nthreadsx) {
          ...
          atomic_add(&outputs[output_offset + (local_ind * num_cols) + i],
                     val,
                     pe);
        }
      }
...
template <typename DataType>
void ScatterNVSHMEM<DataType>::scatter(const DataType* values,
                                       ...
                                       const size_t output_rows_size)
{
  ...
  const auto buffer_size =
    local_mini_batch_size * output_rows_size * values_cols_size;
  ...
  cudaMemsetAsync(output,
                  0,
                  sizeof(DataType) * output_size,
                  m_stream); // Not sure why this seems to be neccessary
  ...
  Scatter_NVSHMEM_Kernel<<<grid_dims, block_dims, 0, m_stream>>>(
    values,
    indices,
    output_buffer_ptr,
...
template <typename DataType>
void GatherNVSHMEM<DataType>::gather(const DataType* values,
                                     const DataType* indices,
                                     DataType* output,
                                     const size_t local_mini_batch_size,
                                     const size_t values_rows_size,
                                     ...
                                     const size_t output_rows_size)
{
  ...
  const auto buffer_size =
    local_mini_batch_size * values_rows_size * values_cols_size;
  const auto output_size =
    local_mini_batch_size * output_rows_size * values_cols_size;
  ...
  cudaMemset(output,
             0,
             sizeof(DataType) *
               output_size); // Not sure why this seems to be neccessary

  constexpr size_t block_size_x = 32;
  constexpr size_t block_size_y = 16;

  // Copy the local values matrix onto the workspace buffer
  // The values matrix must be in symmetric heap in order to be remotely
  // accessible Change grid dimensions for copy kernel. Will be removed in the
  // furure - S.Z
  ...
  Gather_NVSHMEM_Kernel<<<grid_dims, block_dims, 0, m_stream>>>(
...
#define SCATTER_ETI(T)                                                         \
  template void ScatterNVSHMEM<T>::scatter(const T* values,                    \
                                           const T* indices,                   \
                                           T* output,                          \
                                           const size_t local_mini_batch_size, \
                                           const size_t values_rows_size,      \
...
#define GATHER_ETI(T)                                                          \
  template void GatherNVSHMEM<T>::gather(const T* values,                      \
                                         const T* indices,                     \
                                         T* output,                            \
                                         const size_t local_mini_batch_size,   \
                                         const size_t values_rows_size,        \
