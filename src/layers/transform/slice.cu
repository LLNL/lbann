  ...
  slice_layer<TensorDataType, data_layout::DATA_PARALLEL, El::Device::GPU>& l)
{
  ...
  const auto& local_input =
    dynamic_cast<const LocalMatrix&>(input.LockedMatrix());
  auto sync_info = gpu::get_sync_info(local_input);
  ...
  slice_layer<TensorDataType, data_layout::DATA_PARALLEL, El::Device::GPU>& l)
{
  ...
  // Launch GPU kernel
  const auto& max_output_grad_size =
    (max_output_grad_dims[0] * max_output_grad_dims[1] *
     max_output_grad_dims[2] * max_output_grad_dims[3]);
...
#define PROTO(T)                                                               \
  template class slice_layer<T, data_layout::DATA_PARALLEL, El::Device::GPU>;  \
  template class slice_layer<T, data_layout::MODEL_PARALLEL, El::Device::GPU>
