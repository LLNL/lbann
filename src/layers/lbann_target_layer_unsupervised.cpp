...
void lbann::target_layer_unsupervised::setup(int num_prev_neurons) {
  ...
  Zeros(*WB, NumNeurons, num_prev_neurons+1);

  // Initialize weights
  DistMat weights;
  View(weights, *WB, IR(0,NumNeurons), IR(0,num_prev_neurons));
  ...
  Zeros(*Ds_Temp, num_prev_neurons + 1, m_mini_batch_size); // Ds_Temp holds the product of WB^T * Ds
  Zeros(*Acts, NumNeurons, m_mini_batch_size); //clear up Acts before copying fp_input to it
  Zeros(*WB_D, NumNeurons,num_prev_neurons + 1); //clear up before filling with new results
  Zeros(*Ds, NumNeurons, m_mini_batch_size); //clear up before filling with new results
...
DataType lbann::target_layer_unsupervised::forwardProp(DataType prev_WBL2NormSum) {
  ...
  //Acts is linear transformation of WB * X^T
  Gemm(NORMAL, NORMAL, (DataType) 1., *WB, X, (DataType) 0., *Acts);
  int num_errors = 0;
  //not used
  return num_errors;
}
...
//draw image here for debugging original = DsNext, computed = Acts
