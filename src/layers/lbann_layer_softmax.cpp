...
void lbann::SoftmaxLayer::bp_linearity()
{
  ...
  // Compute error signal from nonlinearity
  // error_signal = (prev_error_signal - prev_error_signal^T activations) * activations
  ...
  for(Int c = 0; c < get_effective_minibatch_size(); c++) {
    ...
    const DataType curr_dot_product = Dot(curr_prev_error_signal, curr_activations);
