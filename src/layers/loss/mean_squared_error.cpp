                  ...
                  El::AbstractMatrix<TensorDataType>& local_contribution)
{
  ...
  for (El::Int col = 0; col < local_width; ++col) {
    ...
    for (El::Int row = 0; row < local_height; ++row) {
      const auto& err =
        (local_prediction(row, col) - local_ground_truth(row, col));
      sum += err * err;
    }
  ...
  El::AbstractMatrix<TensorDataType>& local_gradient_wrt_ground_truth)
{
  ...
  const TensorDataType scale =
    static_cast<TensorDataType>(TensorDataType(2) / height);
...
{
  LBANN_CALIPER_MARK_SCOPE("mean_squared_error_layer::fp_compute");
...
{
  LBANN_CALIPER_MARK_SCOPE("mean_squared_error_layer::bp_compute");
