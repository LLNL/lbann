...
void lbann::greedy_layerwise_autoencoder::insert_mirror(uint32_t layer_index) {
  ...
  reconstruction_layer *mirror_layer = new reconstruction_layer(original_layer->get_data_layout(), mirror_index, comm, opt, m_mini_batch_size, original_layer);
...
void lbann::greedy_layerwise_autoencoder::train(int num_epochs, int evaluation_frequency) {
  ...
  while(m_current_phase < num_phases) {
    ...
    optimizer *opt = create_optimizer();
    reconstruction_layer  *mirror_layer = new reconstruction_layer(original_layer->get_data_layout(), m_phase_end, comm, opt, m_mini_batch_size,original_layer);
    Layer *tmp;
