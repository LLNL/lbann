...
void lbann::stacked_autoencoder::begin_stack(const std::string layer_name,
  ...
  int prev_layer_dim = mid_layer->NumNeurons;
    ...
    if(layer_name == "FullyConnected"){
      Optimizer *new_optimizer = optimizer_fac->create_optimizer(data_dist);
      Layer* new_layer
      ...
      Optimizer *mirror_optimizer = optimizer_fac->create_optimizer(data_dist);
      Layer* mirror_layer
      ...
      m_layers.push_back(mirror_layer);
		...
		// create hiden layer
    if(layer_name == "FullyConnected"){
      Optimizer *hidden_optimizer = optimizer_fac->create_optimizer(data_dist);
      Layer* hidden_layer
        = layer_fac->create_layer<FullyConnectedLayer>("FullyConnected",data_dist,cur_size,
                                                       prev_layer_dim,layer_dim,
                                                       m_mini_batch_size, activation, init,
      ...
      // create mirror layer
      Optimizer *mirror_hidden_optimizer = optimizer_fac->create_optimizer(data_dist);
