...
const conduit::Node & data_store_jag::get_node(int data_id, int tid) const {
...
void data_store_jag::build_index_owner() {
  ...
  const std::string data_dir = add_delimiter(m_data_reader->get_file_dir());
  const std::string sample_list_file = data_dir + m_data_reader->get_data_index_list();

  // get filename for mapping file
  const std::string mappling_file = data_dir + m_data_reader->get_local_file_dir();
