...
bool data_reader_synthetic::fetch_datum(CPUMat& X, int data_id, int mb_idx, thread_pool& io_thread_pool) {
  auto X_v = El::View(X, El::ALL, El::IR(mb_idx, mb_idx + 1));
  fill_matrix(X_v);
  return true;
}

bool data_reader_synthetic::fetch_label(CPUMat& Y, int data_id, int mb_idx, thread_pool& io_thread_pool) {
  if (m_num_labels == 0) {
...
bool data_reader_synthetic::fetch_response(CPUMat& Y, int data_id, int mb_idx, thread_pool& io_thread_pool) {
