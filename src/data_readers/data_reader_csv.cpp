...
csv_reader::csv_reader(int batch_size, int label_col, char separator,
                       int skip_cols, bool shuffle)
  : generic_data_reader(batch_size, shuffle), m_separator(separator),
    m_skip_cols(skip_cols), m_label_col(label_col) {}
...
void csv_reader::load() {
  ...
  if (std::getline(ifs, line)) {
    ...
    if (m_label_col < 0) {
      ...
      m_label_col = m_num_cols - 1 - m_skip_cols;
    }
    if (m_label_col + m_skip_cols >= m_num_cols) {
      throw lbann_exception(
