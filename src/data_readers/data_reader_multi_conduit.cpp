...
#include "lbann/data_readers/data_reader_multi_conduit.hpp"
//#include "lbann/data_store/data_store_merge_samples.hpp"
//#include "lbann/utils/options.hpp"

namespace lbann {

data_reader_multi_conduit::data_reader_multi_conduit(
  std::vector<generic_data_reader*> data_readers,
  bool shuffle) :
  generic_compound_data_reader(data_readers, shuffle) {}

data_reader_multi_conduit::data_reader_multi_conduit(
  const data_reader_multi_conduit& other) :
  generic_compound_data_reader(other) {}
// todo  m_num_samples_psum(other.m_num_samples_psum) {}

data_reader_multi_conduit& data_reader_multi_conduit::operator=(
  const data_reader_multi_conduit& other) {
...
#if 0
todo
void data_reader_multi_conduit::load_using_data_store() {
...
#if 0
size_t data_reader_multi_conduit::compute_num_samples_psum() {
...
#if 0
void data_reader_multi_conduit::sanity_check_for_consistency(
  int num_labels, int data_size, int label_size, const std::vector<int> &data_dims) {
  for (auto&& reader : m_data_readers) {
    if (num_labels != reader->get_num_labels()) {
      throw lbann_exception(
        "data_reader_multi_conduit: data readers do not have the same number of labels");
    }
    if (data_size != reader->get_linearized_data_size()) {
      throw lbann_exception(
        "data_reader_multi_conduit: data readers do not have the same data size");
    }
    if (label_size != reader->get_linearized_label_size()) {
      throw lbann_exception(
        "data_reader_multi_conduit: data readers do not have the same label size");
    }
    if (data_dims != reader->get_data_dims()) {
      throw lbann_exception(
        "data_reader_multi_conduit: data readers do not have the same data dims");
    }
  }
}
#endif
...
#if 0
void data_reader_multi_conduit::setup_indices(int num_samples) {
  // Set up our indices.
...
void data_reader_multi_conduit::load() {
/* todo
  if (options::get()->has_bool("use_data_store") && options::get()->get_bool("use_data_store")) {
    data_reader_multi_conduit::load_using_data_store();
    return;
  }
*/
...
bool data_reader_multi_conduit::fetch_datum(CPUMat& X, int data_id, int mb_idx,
                                            int tid) {
return false;
#if 0
  ...
  throw lbann_exception(
    "data_reader_multi_conduit: do not have data ID " +
    std::to_string(data_id));
#endif
}

bool data_reader_multi_conduit::fetch_label(CPUMat& Y, int data_id, int mb_idx,
                                            int tid) {
return false; 
#if 0
  // Find the right data reader to delegate to.
...
bool data_reader_multi_conduit::fetch_response(CPUMat& Y, int data_id, int mb_idx,
                                               int tid) {
return false;
#if 0
  // Find the right data reader to delegate to.
...
void data_reader_multi_conduit::setup_data_store(model *m) {
/*
