...
// This comes after all the headers, and is only visible within the current
// implementation file. To make sure, we put '#undef _CN_' at the end of this
// file
...
std::unordered_map<std::string, int>
  data_reader_jag_conduit::m_num_local_readers;
...
data_reader_jag_conduit::data_reader_jag_conduit(
  const data_reader_jag_conduit& rhs)
...
data_reader_jag_conduit::operator=(const data_reader_jag_conduit& rhs)
{
...
bool data_reader_jag_conduit::has_path(
...
void data_reader_jag_conduit::read_node(
  const data_reader_jag_conduit::file_handle_t& h,
...
#else
bool data_reader_jag_conduit::has_path(const hid_t& h,
                                       const std::string& path) const
{
...
void data_reader_jag_conduit::read_node(const hid_t& h,
                                        const std::string& path,
                                        conduit::Node& n) const
{
...
const conduit::Node&
data_reader_jag_conduit::get_conduit_node(const conduit::Node& n_base,
                                          const std::string key)
{
  return n_base[key];
}

bool data_reader_jag_conduit::load_conduit_node(const size_t i,
                                                const std::string& key,
                                                conduit::Node& node) const
{
  if (m_io_thread_pool != nullptr &&
      m_using_random_node.count(m_io_thread_pool->get_local_thread_id())) {
    LBANN_ERROR("previously retrieved a random conduit node from data_store, "
  ...
  if (!has_path(h, path)) {
    ...
    if (m_data_store != nullptr) {
      if (!m_data_store->is_fully_loaded()) {
        ...
        std::cout
          << get_type() +
               ":: replacing with random node, since failed to open file "
          << file_name << " for sample " << sample_name << " and key: " << key
          << "\n";
        return false;
      }
      else {
...
bool data_reader_jag_conduit::has_conduit_path(const size_t i,
...
void data_reader_jag_conduit::set_independent_variable_type(
  const std::vector<std::vector<data_reader_jag_conduit::variable_t>>&
    independent)
{
 ...
 */
void data_reader_jag_conduit::set_image_dims(const int width,
                                             const int height,
                                             const int ch)
{
 ...
 *       src/proto/init_image_data_readers.cpp
 */
void data_reader_jag_conduit::set_image_choices(
  const std::vector<std::string> image_keys)
{
  m_emi_image_keys = image_keys;
  // For example, in the data reader prototext file, have a line similar to the
  // one below image_keys: ["(0.0, 0.0)/0.0","(90.0,
...
data_reader_jag_conduit::get_image_choices() const
{
...
{
  m_scalar_prefix_filter.push_back(
    (p.first.length() > p.second) ? prefix_t(p.first, p.first.length()) : p);
}
...
{
  m_input_prefix_filter.push_back(
    (p.first.length() > p.second) ? prefix_t(p.first, p.first.length()) : p);
}
 ...
 */
bool data_reader_jag_conduit::filter(
  const std::set<std::string>& key_filter,
  const std::vector<data_reader_jag_conduit::prefix_t>& prefix_filter,
  const std::string& key) const
{
 ...
 *       src/proto/init_image_data_readers.cpp
 */
void data_reader_jag_conduit::set_scalar_choices(
...
const std::vector<std::string>&
data_reader_jag_conduit::get_scalar_choices() const
{
 ...
 *       src/proto/init_image_data_readers.cpp
 */
void data_reader_jag_conduit::set_input_choices(
  const std::vector<std::string>& keys)
{
  m_input_keys = keys;
  check_input_keys();
...
const std::vector<std::string>&
data_reader_jag_conduit::get_input_choices() const
{
  return m_input_keys;
}
...
void data_reader_jag_conduit::check_image_data()
{
  ...
  if (!has_conduit_path(first_idx, "")) {
    LBANN_ERROR(_CN_ + ":: check_image_data() : no sample by " +
                m_sample_list[first_idx].second);
    return;
  }
  ...
  load_conduit_node(first_idx,
                    m_output_image_prefix + m_emi_image_keys[0],
                    n_image);
  conduit_ch_t emi = n_image.value();

  if (m_image_linearized_size !=
      static_cast<size_t>(emi.number_of_elements())) {
  ...
  if (m_image_normalization_params.empty()) {
    m_image_normalization_params.assign(m_emi_image_keys.size() *
                                          m_image_num_channels,
                                        linear_transform_t(1.0, 0.0));
  }
  else if (m_image_normalization_params.size() !=
           static_cast<size_t>(m_image_num_channels)) {
    LBANN_ERROR(_CN_ +
                ":: Incorrect number of image normalization parameter sets!" +
  ...
  for (size_t i = 0u, s = 0u; s < m_emi_image_keys.size(); ++s) {
    for (int c = 0; c < m_image_num_channels; ++c) {
      const auto& param =
        m_image_normalization_params[i * m_image_num_channels + c];
...
void data_reader_jag_conduit::check_scalar_keys()
{
  ...
  for (size_t i = 0u; i < m_scalar_keys.size(); ++i) {
    std::set<std::string>::const_iterator it =
      keys_conduit.find(m_scalar_keys[i]);
  ...
  if (m_scalar_normalization_params.empty()) {
    m_scalar_normalization_params.assign(m_scalar_keys.size(),
                                         linear_transform_t(1.0, 0.0));
  }
  else if (m_scalar_normalization_params.size() != m_scalar_keys.size()) {
    LBANN_ERROR(_CN_ +
                ":: Incorrect number of scalar normalization parameter sets! " +
...
void data_reader_jag_conduit::check_input_keys()
{
  ...
  while (itr.has_next()) {
    ...
    keys_conduit.insert(
      std::pair<std::string, TypeID>(itr.name(),
                                     static_cast<TypeID>(n.dtype().id())));
  }

  bool is_input_t = true;

  for (size_t i = 0u; i < m_input_keys.size(); ++i) {
    std::map<std::string, TypeID>::const_iterator it =
      keys_conduit.find(m_input_keys[i]);
    if (it != keys_conduit.cend()) {
      num_found++;
      found[i] = true;
  ...
  if (m_input_normalization_params.empty()) {
    m_input_normalization_params.assign(m_input_keys.size(),
                                        linear_transform_t(1.0, 0.0));
  }
  else if (m_input_normalization_params.size() != m_input_keys.size()) {
    LBANN_ERROR(_CN_ +
                ":: Incorrect number of input normalization parameter sets! " +
...
void data_reader_jag_conduit::do_preload_data_store()
{
  ...
  for (size_t idx = 0; idx < m_shuffled_indices.size(); idx++) {
    ...
    try {
      ...
      LBANN_ERROR(" :: trying to load the node " + std::to_string(index) +
  ...
  if (get_comm()->am_world_master() ||
    ...
    msg << " loading data for role: " << get_role() << " took "
        << get_time() - tm1 << "s";
...
void data_reader_jag_conduit::load_list_of_samples_from_archive(
...
size_t data_reader_jag_conduit::get_linearized_size(
  const data_reader_jag_conduit::variable_t t) const
{
  switch (t) {
  case JAG_Image:
...
std::vector<size_t>
data_reader_jag_conduit::get_linearized_response_sizes() const
{
  std::vector<size_t> all_dim;
...
const std::vector<int> data_reader_jag_conduit::get_dims(
  const data_reader_jag_conduit::variable_t t) const
{
  switch (t) {
  case JAG_Image:
    return {static_cast<int>(get_num_img_srcs()),
            m_image_height,
            m_image_width};
  ...
  default: { // includes Undefined case
    LBANN_ERROR(_CN_ +
                ":: get_dims() : " + "unknown or undefined variable type");
  }
  }
  return {};
}
...
std::vector<El::Int>
data_reader_jag_conduit::get_slice_points(const slice_points_mode var_category,
                                          bool& is_supported)
{
  ...
  if (var_category == slice_points_mode::INDEPENDENT) {
    ...
    LBANN_ERROR("Unknown variable category \"" +
                lbann::to_string(var_category) +
                "\". Must be either \"independent\" or \"dependent\".");
  }
  return slice_points;
}

std::vector<El::Int> data_reader_jag_conduit::get_slice_points_impl(
...
std::vector<El::Int>
data_reader_jag_conduit::get_slice_points_independent() const
{
...
std::string data_reader_jag_conduit::to_string(
  const std::vector<data_reader_jag_conduit::variable_t>& vec)
{
  std::string str("[");
...
std::string data_reader_jag_conduit::to_string(
  const std::vector<std::vector<data_reader_jag_conduit::variable_t>>& vec)
{
  std::string str("[");
  for (const auto& el : vec) {
    str += ' ' + data_reader_jag_conduit::to_string(el);
...
{
  std::string ret =
    std::string("data_reader_jag_conduit:\n") + " - independent: " +
    data_reader_jag_conduit::to_string(m_independent_groups) + "\n" +
    " - dependent: " + data_reader_jag_conduit::to_string(m_dependent_groups) +
    "\n" + " - images: " + std::to_string(m_num_img_srcs) + " of " +
    std::to_string(m_image_num_channels) + 'x' + std::to_string(m_image_width) +
    'x' + std::to_string(m_image_height) + "\n" +
...
bool data_reader_jag_conduit::check_non_numeric(const std::string key)
{
  ...
  if (kit != non_numeric_vars.cend()) {
    std::string err = "data_reader_jag_conduit::add_val() : non-numeric '" +
                      key + "' requires a conversion method.";
#if 1
...
data_reader_jag_conduit::get_image_data(const size_t sample_id,
                                        conduit::Node& sample) const
{
  ...
  for (const auto& emi_tag : m_emi_image_keys) {
    ...
    const std::string conduit_obj =
      LBANN_DATA_ID_STR(sample_id) + conduit_field;
    if (sample[conduit_obj].schema().dtype().is_empty()) {
      if (data_store_active()) {
        LBANN_ERROR("Unable to find field ",
                    conduit_obj,
                    " in conduit node: ",
                    std::to_string(sample_id),
                    ": num nodes successfully loaded by this rank: ",
                    m_data_store->get_data_size(),
                    " num successful calls to get_image_data on this rank: ");
      }
      conduit::Node n_image;
...
std::vector<data_reader_jag_conduit::scalar_t>
data_reader_jag_conduit::get_scalars(const size_t sample_id,
                                     conduit::Node& sample) const
{
  ...
  for (const auto& key : m_scalar_keys) {
    ...
    const scalar_t val_raw =
      static_cast<scalar_t>(sample[conduit_obj].to_value());
    const scalar_t val =
      static_cast<scalar_t>(val_raw * tr->first + tr->second);
...
std::vector<data_reader_jag_conduit::input_t>
data_reader_jag_conduit::get_inputs(const size_t sample_id,
                                    conduit::Node& sample) const
{
  ...
  // automatically determine which method to use based on if all the variables
  // are of input_t
  if (m_uniform_input_type) {
    // avoid some overhead by taking advantage of the fact that all the
    ...
    for (const auto& key : m_input_keys) {
      ...
      const input_t val =
        static_cast<input_t>(val_raw * tr->first + tr->second);
  ...
  const std::string tag)
{
  ...
  case JAG_Image: {
    ...
    std::vector<std::vector<DataType>> img_data(
      get_image_data(data_id, sample));

    if (img_data.size() != num_images) {
      LBANN_ERROR(
        _CN_ + ":: fetch() : the number of images is not as expected " +
        std::to_string(img_data.size()) + "!=" + std::to_string(num_images));
    }
    if (!m_split_channels && m_image_num_channels != 1) {
      LBANN_ERROR(_CN_ +
                  ":: fetch() : transform pipeline now requires single channel "
                  "images: num_channels=" +
                  std::to_string(m_image_num_channels) +
                  " split_channel=" + std::to_string(m_split_channels));
    }

    std::vector<size_t> dims = {num_channels,
                                static_cast<size_t>(m_image_height),
                                static_cast<size_t>(m_image_width)};
    std::vector<size_t> ch_dims = {static_cast<size_t>(m_image_height),
                                   static_cast<size_t>(m_image_width)};
    ...
    for (size_t i = 0u; i < num_images; ++i) {
      ...
      const std::vector<size_t> ch_sizes(num_channels,
                                         m_image_height * m_image_width);
  ...
  case JAG_Scalar: {
    ...
    set_minibatch_item<scalar_t>(X,
                                 mb_idx,
                                 scalars.data(),
                                 get_linearized_scalar_size());
    break;
  }
  case JAG_Input: {
    const std::vector<input_t> inputs(get_inputs(data_id, sample));
    set_minibatch_item<input_t>(X,
                                mb_idx,
                                inputs.data(),
                                get_linearized_input_size());
    break;
  }
  default: { // includes Undefined case
    LBANN_ERROR(_CN_ + ":: fetch_" + tag +
                "() : unknown or undefined variable type");
  }
  }
  return true;
...
bool data_reader_jag_conduit::fetch_datum(CPUMat& X, int data_id, int mb_idx)
{
  ...
  for (size_t i = 0u; ok && (i < X_v.size()); ++i) {
    // The third argument mb_idx below is 0 because it is for the view of X not
    // X itself
...
void data_reader_jag_conduit::add_image_normalization_param(
  const data_reader_jag_conduit::linear_transform_t& t)
{
  m_image_normalization_params.push_back(t);
}

void data_reader_jag_conduit::add_scalar_normalization_param(
  const data_reader_jag_conduit::linear_transform_t& t)
{
  m_scalar_normalization_params.push_back(t);
}

void data_reader_jag_conduit::add_input_normalization_param(
  const data_reader_jag_conduit::linear_transform_t& t)
{
