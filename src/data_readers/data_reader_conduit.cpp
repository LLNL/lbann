...
#define _CN_ "data_reader_conduit"

namespace lbann {

std::unordered_map<std::string, int> data_reader_conduit::m_num_local_readers;

const std::set<std::string> data_reader_conduit::non_numeric_vars = {
  "fusion_reaction",
...
void data_reader_conduit::set_io_buffer_type(const std::string io_buffer) {
  m_io_buffer_type = io_buffer;
}

void data_reader_conduit::set_local_id(const std::string role) {
  m_local_reader_id = m_num_local_readers[role]++;
}

int data_reader_conduit::get_local_id(const std::string role) const {
  return m_local_reader_id;
}

void data_reader_conduit::set_leading_reader(data_reader_conduit* r) {
  m_leading_reader = r;
}

data_reader_conduit* data_reader_conduit::get_leading_reader() {
  return m_leading_reader;
}

void data_reader_conduit::shuffle_indices(rng_gen& gen) {
...
int data_reader_conduit::compute_max_num_parallel_readers() {
...
bool data_reader_conduit::check_num_parallel_readers(long data_set_size) {
  return true;
}

data_reader_conduit::data_reader_conduit(const std::shared_ptr<cv_process>& pp, bool shuffle)
...
void data_reader_conduit::copy_members(const data_reader_conduit& rhs, const std::vector<int>& ds_sample_move_list) {
...
data_reader_conduit::data_reader_conduit(bool shuffle) 
  : generic_data_reader(shuffle) {
  set_defaults();
}

data_reader_conduit::data_reader_conduit(const data_reader_conduit& rhs)
  : generic_data_reader(rhs) {
  copy_members(rhs);
}

data_reader_conduit::data_reader_conduit(const data_reader_conduit& rhs, const std::vector<int>& ds_sample_move_list)
  : generic_data_reader(rhs) {
  copy_members(rhs, ds_sample_move_list);
}

data_reader_conduit& data_reader_conduit::operator=(const data_reader_conduit& rhs) {
...
data_reader_conduit::~data_reader_conduit() {
  // if (m_data_store != nullptr) {
  //   delete m_data_store;
  // }
}

void data_reader_conduit::set_defaults() {
...
void data_reader_conduit::setup(int num_io_threads, std::shared_ptr<thread_pool> io_thread_pool) {
...
bool data_reader_conduit::replicate_processor(const cv_process& pp, const int nthreads) {
...
const conduit::Node& data_reader_conduit::get_conduit_node(const conduit::Node& n_base, const std::string key) {
  return n_base[key];
}

bool data_reader_conduit::load_conduit_node(const size_t i, const std::string& key, conduit::Node& node) const {
...
bool data_reader_conduit::has_conduit_path(const size_t i, const std::string& key) const {
  const sample_t& s = m_sample_list[i];
  sample_file_id_t id = s.first;
  const std::string& sample_name = s.second;
...
void data_reader_conduit::set_independent_variable_type(
  const std::vector< std::vector<data_reader_conduit::variable_t> >& independent) {
...
void data_reader_conduit::add_independent_variable_type(
  const data_reader_conduit::variable_t independent) {
...
void data_reader_conduit::set_dependent_variable_type(
  const std::vector< std::vector<data_reader_conduit::variable_t> >& dependent) {
...
void data_reader_conduit::add_dependent_variable_type(
  const data_reader_conduit::variable_t dependent) {
...
std::vector<data_reader_conduit::variable_t>
data_reader_conduit::get_independent_variable_type() const {
  return m_independent;
}

std::vector<data_reader_conduit::variable_t>
data_reader_conduit::get_dependent_variable_type() const {
  return m_dependent;
}

void data_reader_conduit::set_image_dims(const int width, const int height, const int ch) {
...
void data_reader_conduit::set_image_choices(const std::vector<std::string> image_keys) {
...
const std::vector<std::string>& data_reader_conduit::get_image_choices() const {
  return m_emi_image_keys;
}


void data_reader_conduit::add_scalar_filter(const std::string& key) {
  m_scalar_filter.insert(key);
}

void data_reader_conduit::add_scalar_prefix_filter(const prefix_t& p) {
  m_scalar_prefix_filter.push_back((p.first.length() > p.second)? prefix_t(p.first, p.first.length()) : p);
}

void data_reader_conduit::add_input_filter(const std::string& key) {
  m_input_filter.insert(key);
}

void data_reader_conduit::add_input_prefix_filter(const prefix_t& p) {
 ...
 */
bool data_reader_conduit::filter(const std::set<std::string>& key_filter,
  const std::vector<data_reader_conduit::prefix_t>& prefix_filter, const std::string& key) const {
...
void data_reader_conduit::set_scalar_choices(const std::vector<std::string>& keys) {
  m_scalar_keys = keys;
  check_scalar_keys();
}

void data_reader_conduit::set_all_scalar_choices() {
...
const std::vector<std::string>& data_reader_conduit::get_scalar_choices() const {
 ...
 */
void data_reader_conduit::set_input_choices(const std::vector<std::string>& keys) {
  m_input_keys = keys;
  check_input_keys();
}

void data_reader_conduit::set_all_input_choices() {
...
const std::vector<std::string>& data_reader_conduit::get_input_choices() const {
  return m_input_keys;
}


void data_reader_conduit::set_linearized_image_size() {
...
void data_reader_conduit::check_scalar_keys() {
  //@TODO revisit later -- don't know how to handle this yet
  if (m_data_store != nullptr) {
    return;
  }
...
void data_reader_conduit::preload_data_store() {
...
void data_reader_conduit::load_list_of_samples(const std::string sample_list_file, size_t stride, size_t offset) {
...
void data_reader_conduit::load_list_of_samples_from_archive(const std::string& sample_list_archive) {
  // load the sample list
  double tm1 = get_time();
...
unsigned int data_reader_conduit::get_num_img_srcs() const {
  return m_num_img_srcs;
}

size_t data_reader_conduit::get_linearized_image_size() const {
  return m_image_linearized_size;
}

size_t data_reader_conduit::get_linearized_1ch_image_size() const {
  return m_1ch_image_linearized_size;
}

size_t data_reader_conduit::get_linearized_scalar_size() const {
  return m_scalar_keys.size();
}

size_t data_reader_conduit::get_linearized_input_size() const {
  return m_input_keys.size();
}


size_t data_reader_conduit::get_linearized_size(const data_reader_conduit::variable_t t) const {
  switch (t) {
    case JAG_Image:
...
int data_reader_conduit::get_linearized_data_size() const {
  size_t sz = 0u;
...
int data_reader_conduit::get_linearized_response_size() const {
  size_t sz = 0u;
...
std::vector<size_t> data_reader_conduit::get_linearized_data_sizes() const {
...
std::vector<size_t> data_reader_conduit::get_linearized_response_sizes() const {
  std::vector<size_t> all_dim;
...
const std::vector<int> data_reader_conduit::get_dims(const data_reader_conduit::variable_t t) const {
  switch (t) {
    case JAG_Image:
...
const std::vector<int> data_reader_conduit::get_data_dims() const {
#if 1
...
std::vector<El::Int> data_reader_conduit::get_slice_points(const std::vector< std::vector<data_reader_conduit::variable_t> >& var) const {
...
std::vector<El::Int> data_reader_conduit::get_slice_points_independent() const {
  return get_slice_points(m_independent_groups);
}

std::vector<El::Int> data_reader_conduit::get_slice_points_dependent() const {
  return get_slice_points(m_independent_groups);
}

int data_reader_conduit::get_num_data() const {

  return (int)m_shuffled_indices.size();
}

int data_reader_conduit::get_num_labels() const {
  return m_num_labels;
}

int data_reader_conduit::get_linearized_label_size() const {
  return m_num_labels;
}

int data_reader_conduit::get_linearized_size(const std::string& desc) const {
  if (desc == "JAG_Image") {
...
void data_reader_conduit::set_split_image_channels() {
  m_split_channels = true;
}

void data_reader_conduit::unset_split_image_channels() {
  m_split_channels = false;
}

bool data_reader_conduit::check_split_image_channels() const {
  return m_split_channels;
}


std::string data_reader_conduit::to_string(const variable_t t) {
  switch (t) {
    case Undefined:  return "Undefined";
    case JAG_Image:  return "JAG_Image";
    case JAG_Scalar: return "JAG_Scalar";
    case JAG_Input:  return "JAG_Input";
    case JAG_TimeSeries:  return "JAG_TimeSeries";
  }
  return "Undefined";
}

std::string data_reader_conduit::to_string(const std::vector<data_reader_conduit::variable_t>& vec) {
  std::string str("[");
  for (const auto& el: vec) {
    str += ' ' + data_reader_conduit::to_string(el);
  }
  str += " ]";
  return str;
}

std::string data_reader_conduit::to_string(const std::vector< std::vector<data_reader_conduit::variable_t> >& vec) {
  std::string str("[");
  for (const auto& el: vec) {
    str += ' ' + data_reader_conduit::to_string(el);
  }
...
std::string data_reader_conduit::get_description() const {
  std::stringstream leading_reader;
  leading_reader << m_leading_reader;
  std::string ret = std::string("data_reader_conduit:\n")
    + " - independent: " + data_reader_conduit::to_string(m_independent_groups) + "\n"
    + " - dependent: " + data_reader_conduit::to_string(m_dependent_groups) + "\n"
...
bool data_reader_conduit::check_non_numeric(const std::string key) {
  std::set<std::string>::const_iterator kit = non_numeric_vars.find(key);
  if (kit != non_numeric_vars.cend()) {
    std::string err = "data_reader_conduit::add_val() : non-numeric '" + key
...
void data_reader_conduit::image_normalization(cv::Mat& img, size_t i, size_t ch) const {
...
std::vector<CPUMat>
data_reader_conduit::create_datum_views(CPUMat& X, const std::vector<size_t>& sizes, const int mb_idx) const {
...
int data_reader_conduit::reuse_data(CPUMat& X) {
  El::Copy(m_data_cache, X);
  return m_cached_data_mb_size;
}

int data_reader_conduit::reuse_responses(CPUMat& Y) {
  El::Copy(m_response_cache, Y);
  return m_cached_response_mb_size;
}

int data_reader_conduit::reuse_labels(CPUMat& Y) {
  El::Copy(m_label_cache, Y);
  return m_cached_label_mb_size;
}

int data_reader_conduit::fetch_data(CPUMat& X, El::Matrix<El::Int>& indices_fetched) {
  if ((m_leading_reader != this) && (m_leading_reader != nullptr)) {
...
int data_reader_conduit::fetch_responses(CPUMat& Y) {
  if ((m_leading_reader != this) && (m_leading_reader != nullptr)) {
...
int data_reader_conduit::fetch_labels(CPUMat& Y) {
  if ((m_leading_reader != this) && (m_leading_reader != nullptr)) {
...
void data_reader_conduit::setup_data_store(int mini_batch_size) {
   if (m_data_store != nullptr) {
     m_data_store->setup(mini_batch_size);
   }
}

void data_reader_conduit::save_image(Mat& pixels, const std::string filename, bool do_scale) {
  internal_save_image(pixels, filename, m_image_height, m_image_width, 1, do_scale);
}

void data_reader_conduit::print_schema(const size_t sample_id) const {
  //@TODO revisit later -- don't know how to handle this yet
  if (m_data_store != nullptr) {
    return;
  }
...
void data_reader_conduit::clear_image_normalization_params() {
  m_image_normalization_params.clear();
}

void data_reader_conduit::clear_scalar_normalization_params() {
  m_scalar_normalization_params.clear();
}

void data_reader_conduit::clear_input_normalization_params() {
  m_input_normalization_params.clear();
}

void data_reader_conduit::add_image_normalization_param(const data_reader_conduit::linear_transform_t& t) {
  m_image_normalization_params.push_back(t);
}

void data_reader_conduit::add_scalar_normalization_param(const data_reader_conduit::linear_transform_t& t) {
  m_scalar_normalization_params.push_back(t);
}

void data_reader_conduit::add_input_normalization_param(const data_reader_conduit::linear_transform_t& t) {
