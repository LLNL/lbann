                            ...
                            generic_data_reader*& reader)
{
  ...
  for (int i = 0; i < pb_readme.transforms_size(); ++i) {
    ...
    if (trans.has_center_crop()) {
      ...
      height =
        trans.random_resized_crop_with_fixed_aspect_ratio().crop_height();
  ...
  if (name == "imagenet") {
    ...
    std::vector<std::vector<var_t>> independent_type(
      pb_schema.independent_size());

    for (int i = 0; i < pb_schema.independent_size(); ++i) {
      const lbann_data::DataSetMetaData::Schema::JAGDataSlice& slice =
        pb_schema.independent(i);
    ...
    const int num_scalar_prefix_filters =
      pb_schema.jag_scalar_prefix_filters_size();
    for (int i = 0; i < num_scalar_prefix_filters; ++i) {
      using prefix_t = lbann::data_reader_jag_conduit::prefix_t;
      const prefix_t pf =
        std::make_pair(pb_schema.jag_scalar_prefix_filters(i).key_prefix(),
    ...
    // add scalar output key prefixes to filter out by
    const int num_input_prefix_filters =
      pb_schema.jag_input_prefix_filters_size();
    ...
    const lbann_data::DataSetMetaData::Normalization& pb_normalization =
      pb_metadata.normalization();
    // add image normalization parameters
    const int num_image_normalization_params =
      pb_normalization.jag_image_normalization_params_size();
    for (int i = 0; i < num_image_normalization_params; ++i) {
      using linear_transform_t =
        lbann::data_reader_jag_conduit::linear_transform_t;
      const linear_transform_t np = std::make_pair(
        pb_normalization.jag_image_normalization_params(i).scale(),
    ...
    const int num_scalar_normalization_params =
      pb_normalization.jag_scalar_normalization_params_size();
    for (int i = 0; i < num_scalar_normalization_params; ++i) {
      using linear_transform_t =
        lbann::data_reader_jag_conduit::linear_transform_t;
      const linear_transform_t np = std::make_pair(
        pb_normalization.jag_scalar_normalization_params(i).scale(),
    ...
    const int num_input_normalization_params =
      pb_normalization.jag_input_normalization_params_size();
    for (int i = 0; i < num_input_normalization_params; ++i) {
      using linear_transform_t =
        lbann::data_reader_jag_conduit::linear_transform_t;
      const linear_transform_t np = std::make_pair(
        pb_normalization.jag_input_normalization_params(i).scale(),
    ...
    reader = reader_jag;
    if (master)
      std::cout << reader->get_type() << " is set" << std::endl;
    return;
  }
  ...
  if (!image_data_reader_ptr && master) {
    ...
    err << __FILE__ << " " << __LINE__
        << " :: invalid image data reader pointer";
                                ...
                                generic_data_reader*& reader)
{
  ...
  if (name == "mnist") {
    ...
    if (master)
      std::cout << "mnist_reader is set" << std::endl;
  }
  else if (name == "cifar10") {
    reader = new cifar10_reader(shuffle);
    if (master)
      std::cout << "cifar10_reader is set" << std::endl;
  }
  else {
    if (master) {
      std::stringstream err;
      err << __FILE__ << " " << __LINE__
          << " :: unknown name for image data reader: " << name;
      throw lbann_exception(err.str());
    }
  }
