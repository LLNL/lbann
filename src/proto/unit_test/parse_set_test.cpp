...
TEST_CASE("Testing parse_set", "[proto][utilities]")
{
  SECTION("execution_mode")
  {
    ...
    CHECK(lbann::parse_set<lbann::execution_mode>("") ==
  ...
  SECTION("std::string")
  {
    ...
    auto const answer = lbann::parse_set<std::string>("this is a test");
    CHECK(answer == expected);
  ...
  SECTION("int")
  {
    ...
    auto const answer = lbann::parse_set<int>("1 1 2 1 3 4 3 3 5 2");
    CHECK(answer == expected);
