...
data_store_conduit::data_store_conduit(generic_data_reader* reader)
  : m_reader(reader)
{
  ...
  m_partition_in_trainer =
    m_rank_in_trainer /
    num_io_parts; // needs a better name  which group you are in
  ...
  m_num_partitions_in_trainer =
    m_np_in_trainer / num_io_parts; // rename this m_num_io_groups_in_trainer
  ...
  if (arg_parser.get<bool>(LBANN_OPTION_DATA_STORE_FAIL)) {
    LBANN_ERROR("data_store_conduit is throwing a fake exception; this is for "
                "use during testing");
  }
  ...
  if (arg_parser.get<std::string>(LBANN_OPTION_DATA_STORE_TEST_CHECKPOINT) !=
    ...
    LBANN_ERROR(
      "you passed both --data_store_test_checkpoint and --data_store_spill; "
      "please use one or the other or none, but not both");
  }
  if (arg_parser.get<std::string>(LBANN_OPTION_DATA_STORE_TEST_CHECKPOINT) !=
      "") {
...
void data_store_conduit::setup_checkpoint_test()
{
  auto& arg_parser = global_argument_parser();
  std::string c =
    arg_parser.get<std::string>(LBANN_OPTION_DATA_STORE_TEST_CHECKPOINT);
  if (c == "lassen") {
...
std::string data_store_conduit::get_lassen_spill_dir()
{
  ...
  if (val == NULL) {
    LBANN_ERROR(
      "std::getenv(\"BBPATH\") returned NULL; unable to use burst buffer");
  }
  std::string cc(val);
                                                    ...
                                                    uint64_t data_id)
{
  ...
  }
  {
    ...
    if (m_compacted_sample_size == 0) {
      ...
      PROFILE("num bytes for nodes to be transmitted: ",
              nd.total_bytes_compact(),
              " per node");
    }
    else if (m_compacted_sample_size != nd.total_bytes_compact() &&
             !m_node_sizes_vary) {
      LBANN_ERROR("Conduit node being added data_id: ",
                  data_id,
                                          ...
                                          bool already_have)
{
  ...
  if (is_local_cache() && is_preloading()) {
    LBANN_ERROR("you called data_store_conduit::set_conduit_node, but you're "
                "running in local cache mode with preloading; something is "
                "broken; please contact Dave Hysom");
  }

  {
    // std::lock_guard<std::mutex> lock(m_mutex);
    if (already_have == false && m_data.find(data_id) != m_data.end()) {
      DEBUG_DS("m_data.size: ",
               m_data.size(),
               " ERROR: duplicate data_id: ",
               data_id);
      LBANN_ERROR("duplicate data_id: ",
                  data_id,
                  " in data_store_conduit::set_conduit_node; role: ",
                  m_reader->get_role());
    }
  }
  ...
  if (already_have && is_local_cache()) {
    if (m_data.find(data_id) == m_data.end()) {
      LBANN_ERROR("you claim the passed node was obtained from this "
                  "data_store, but the data_id (",
                  data_id,
                  ") doesn't exist in m_data");
    }
    return;
  }
  ...
  if (is_local_cache()) {
    ...
    if (m_spill) {
      ...
      //      m_mutex.lock();
      DEBUG_DS("set_conduit_node : rank_in_trainer=",
               m_rank_in_trainer,
               " and partition_in_trainer=",
               m_partition_in_trainer,
               " offset in partition=",
...
data_store_conduit::get_conduit_node(uint64_t data_id) const
{
  ...
  if (t2 == m_minibatch_data.end()) {
    ...
    LBANN_ERROR("failed to find data_id: ",
                data_id,
                " in m_minibatch_data; m_minibatch_data.size: ",
                m_minibatch_data.size(),
                " and also failed to find it in m_data; m_data.size: ",
                m_data.size(),
                "; role: ",
                m_reader->get_role());
  }
...
void data_store_conduit::build_node_for_sending(const conduit::Node& node_in,
                                                  ...
                                                  uint64_t mb_size)
{
  ...
  int k = 0;
  DEBUG_DS("build_indices_i_will_send; cur pos: ",
           current_pos,
           " mb_size: ",
           mb_size,
           " m_data.size: ",
  ...
  for (uint64_t i = current_pos; i < current_pos + mb_size; i++) {
    ...
    if (is_mine) {
      ...
      int num_ranks_in_partition = 1;
#endif // LBANN_HAS_DISTCONV
      m_indices_to_send[(((i % m_owner_map_mb_size) %
                          m_num_partitions_in_trainer) *
                           num_ranks_in_partition +
                         m_offset_in_partition)]
        .insert(index);
...
void data_store_conduit::build_preloaded_owner_map(
...
const conduit::Node& data_store_conduit::get_random_node() const
{
  ...
  // Deal with edge case
  if (sz == 0) {
    LBANN_ERROR("can't return random node since we have no data "
                "(set_conduit_node has never been called)");
  }
...
const conduit::Node&
data_store_conduit::get_random_node(const std::string& field) const
{
...
int data_store_conduit::get_index_owner(int idx)
{
  ...
  if (m_owner.find(key) == m_owner.end()) {
    LBANN_ERROR(" idx: ",
                idx,
                " was not found in the m_owner map; map size: ",
                m_owner.size());
  }
  return m_owner[key];
}

void data_store_conduit::check_mem_capacity(lbann_comm* comm,
  ...
  if (comm->am_world_master()) {
      ...
      if (line.find("MemAvailable:")) {
        ...
        if (units != "kB") {
          LBANN_ERROR(
            "units is ",
            units,
            " but we only know how to handle kB; please contact Dave Hysom");
        }
        break;
      }
    }
    ...
    while (std::getline(istr, line)) {
      ...
      if (!got_one) {
        hid_t hdf5_file_hnd;
        try {
          hdf5_file_hnd = conduit::relay::io::hdf5_open_file_for_read(
            base_dir + '/' + filename);
        }
    ...
    if (m_rank_in_trainer == k) {
      ...
      for (size_t i = 0; i < others.size(); i += 2) {
        if (m_sample_sizes.find(others[i]) != m_sample_sizes.end()) {
          ...
          LBANN_ERROR("m_sample_sizes.find(others[i]) != m_sample_sizes.end() "
                      "for data_id: ",
                      others[i]);
        }
...
void data_store_conduit::set_is_preloading(bool flag) { m_preloading = flag; }
...
void data_store_conduit::set_is_explicitly_loading(bool flag)
{
  ...
  if (is_preloading() && is_explicitly_loading()) {
    LBANN_ERROR(
      "flags for both explicit and pre- loading are set; this is an error");
  }
}
...
void data_store_conduit::get_image_sizes(map_is_t& file_sizes,
                                         std::vector<std::vector<int>>& indices)
{
  // get list of image file names
  image_data_reader* image_reader = dynamic_cast<image_data_reader*>(m_reader);
  if (image_reader == nullptr) {
    LBANN_ERROR("data_reader_image *image_reader = "
                "dynamic_cast<data_reader_image*>(m_reader) failed");
  }
  ...
  if (is_explicitly_loading()) {
    ...
    for (const auto& t : m_data) {
      ...
      my_image_sizes.push_back(
        t.second[LBANN_DATA_ID_STR(data_id) + "/buffer_size"].value());
    }
  }

  else {
    // get sizes of files for which I'm responsible
    for (size_t h = m_rank_in_trainer; h < m_shuffled_indices->size();
         h += m_np_in_trainer) {
      ++m_my_num_indices;
  ...
  m_comm->all_gather<int>(&my_count,
                          1,
                          counts.data(),
                          1,
                          m_comm->get_trainer_comm());
...
void data_store_conduit::compute_image_offsets(
  map_is_t& sizes,
  std::vector<std::vector<int>>& indices)
{
  std::lock_guard<std::mutex> lock(m_mutex);
...
void data_store_conduit::allocate_shared_segment(
  map_is_t& sizes,
  std::vector<std::vector<int>>& indices)
{
  off_t size = 0;
  ...
  PROFILE("  Shared Memory segment statistics:\n",
          "   size of required shared memory segment: ",
          utils::commify(m_mem_seg_length),
          "\n",
          "   available mem: ",
          utils::commify(avail_mem),
          "\n",
          "   required size is ",
          percent,
  ...
  if (node_id == 0) {
    ...
    int sanity =
      msync(static_cast<void*>(m_mem_seg), m_mem_seg_length, MS_SYNC);
    if (sanity != 0) {
...
void data_store_conduit::preload_local_cache() { exchange_local_caches(); }
...
void data_store_conduit::read_files(std::vector<char>& work,
                                    map_is_t& sizes,
...
void data_store_conduit::build_conduit_nodes(map_is_t& sizes)
{
  ...
  for (auto t : sizes) {
    ...
    if (m_image_offsets.find(data_id) == m_image_offsets.end()) {
      LBANN_ERROR(
        "m_image_offsets.find(data_id) == m_image_offsets.end() for data_id: ",
        data_id);
    }
...
void data_store_conduit::fillin_shared_images(char* images,
                                              size_t size,
                                              size_t offset)
{
  PROFILE("  fillin_shared_images; size: ",
          utils::commify(size),
          " offset: ",
          utils::commify(offset));
  memcpy(reinterpret_cast<void*>(m_mem_seg + offset),
         reinterpret_cast<const void*>(images),
         size);
}

void data_store_conduit::exchange_images(std::vector<char>& work,
                                         ...
                                         std::vector<std::vector<int>>& indices)
{
  ...
  if (is_explicitly_loading()) {
    ...
    for (const auto& t : m_data) {
      ...
      if (offset2 > work.size()) {
        LBANN_ERROR("offset >= work.size(); offset: ",
                    offset2,
                    " work.size(): ",
                    work.size(),
                    " sz: ",
  ...
  for (int p = 0; p < m_np_in_trainer; p++) {
    ...
    if (n < 0) {
      LBANN_ERROR(
        "(n < 0; that shouldn't be possible; please contact Dave Hysom");
    }
    for (int k = 0; k < n; k++) {
    ...
    */
    ...
    for (size_t i = 0; i < rounds.size(); i++) {
      ...
      if (m_rank_in_trainer == p) {
        m_comm->trainer_broadcast<char>(p,
                                        work.data() + work_vector_offset,
                                        sz);
        if (node_rank == 0) {
...
void data_store_conduit::exchange_owner_maps()
{
  PROFILE("starting exchange_owner_maps;",
          "my owner map size: ",
          m_owner.size());
  DEBUG_DS("starting exchange_owner_maps;", "size: ", m_owner.size());
  ...
  for (auto t : m_owner) {
    ...
    DEBUG_DS("I am building the size vector from the owner map for ",
             t.first.first,
             ".",
             t.first.second,
             " and ",
  ...
  for (int k = 0; k < m_np_in_trainer; k++) {
    ...
    if (m_rank_in_trainer == k) {
      m_comm->broadcast<std::pair<size_t, size_t>>(k,
                                                   nodes_i_own.data(),
                                                   all_counts[k],
                                                   m_comm->get_trainer_comm());
      if (m_debug) {
      ...
      for (size_t i = 0; i < other_ranks_nodes.size(); ++i) {
        ...
        if (m_owner.find(key) != m_owner.end()) {
          ...
          if (m_debug) {
            ...
            DEBUG_DS(
              "data_store_conduit::exchange_owner_maps, duplicate data_id: ",
              slab_id.first,
              ".",
              slab_id.second,
              "; k= ",
            ...
            for (auto t : m_owner)
              DEBUG_DS("data_id: ",
                       ...
                       t.second);
            DEBUG_DS("\nother_ranks_nodes[k]: ");
            for (auto t : other_ranks_nodes)
              DEBUG_DS(t.first, ".", t.second, " ");
          }

          LBANN_ERROR("duplicate data_id: ",
                      other_ranks_nodes[i].first,
                      ".",
                      other_ranks_nodes[i].second,
                      " role: ",
                      m_reader->get_role(),
                      "; m_owner[",
                      other_ranks_nodes[i].first,
...
void data_store_conduit::profile_timing()
{
  ...
  if (m_exchange_time > 0.) {
    PROFILE("\n",
            ...
            m_exchange_time,
            "\n",
            "  exchange sample sizes:    ",
            m_exchange_sample_sizes_time,
            "\n",
            "  start sends and rcvs:     ",
            m_start_snd_rcv_time,
            "\n",
            "  wait alls:                ",
            m_wait_all_time,
            "\n",
            "  unpacking rcvd nodes:     ",
            m_rebuild_time,
            "\n\n");
    ...
    if (arg_parser.get<bool>(LBANN_OPTION_DATA_STORE_MIN_MAX_TIMING)) {
      ...
      if (m_trainer_master) {
        ...
        m_comm->trainer_reduce<double>(send.data(),
                                       count,
                                       rcv_max.data(),
                                       El::mpi::MAX);
        m_comm->trainer_reduce<double>(send.data(),
                                       ...
                                       rcv_min.data(),
                                       El::mpi::MIN);
        PROFILE("Exchange Data MAX Timing:\n",
                "  exchange_mini_batch_data: ",
                rcv_max[0],
                "\n",
                "  exchange sample sizes:    ",
...
void data_store_conduit::test_checkpoint(const std::string& checkpoint_dir)
{
  ...
  if (m_world_master) {
    std::cout << "Here is part of the re-loaded owner map; map.size(): "
              << m_owner.size() << std::endl;
    print_partial_owner_map(10);
  ...
  for (auto t : m_owner) {
    if (sanity.find(t.first) == sanity.end()) {
      LBANN_ERROR("sanity.find(t.first) == sanity.end() for t.first= ",
                  t.first.first,
                  ":",
                  t.first.second);
    }
    else if (sanity[t.first] != m_owner[t.first]) {
      LBANN_ERROR("sanity[t.first] != m_owner[t.first] for t.first= ",
                  t.first.first,
                  ":",
                  t.first.second,
                  " and m_owner[t.first]= ",
...
void data_store_conduit::make_dir_if_it_doesnt_exist(
  const std::string& dir_name)
{
  ...
  if (node_rank == 0) {
    ...
    if (!exists) {
      PROFILE("data_store_conduit; the directory '",
              dir_name,
...
void data_store_conduit::load_checkpoint(std::string dir_name,
                                         generic_data_reader* reader)
{
  ...
  if (!exists) {
    LBANN_ERROR(
      "cannot load data_store from file, since the specified directory ",
      dir_name,
      "doesn't exist");
  }
...
{
  return m_spill_dir_base + "/conduit_" + m_reader->get_role() + "_" +
         std::to_string(m_rank_in_world);
}

std::string data_store_conduit::get_cereal_fn() const
{
  return m_spill_dir_base + '/' + m_cereal_fn + "_" + m_reader->get_role() +
         "_" + std::to_string(m_rank_in_world) +
#ifdef LBANN_HAS_CEREAL_XML_ARCHIVES
         ".xml"
#else  // if defined LBANN_HAS_CEREAL_BINARY_ARCHIVES
         ".bin"
...
{
  return m_spill_dir_base + "/metadata_" + m_reader->get_role() + "_" +
         std::to_string(m_rank_in_world);
}
...
void data_store_conduit::open_next_conduit_spill_directory()
{
  ...
  m_cur_spill_dir_integer += 1;
  m_cur_spill_dir =
    get_conduit_dir() + "/" + std::to_string(m_cur_spill_dir_integer);
...
void data_store_conduit::spill_conduit_node(const conduit::Node& node,
                                            int data_id)
{
  ...
  node.save(fn);
  m_metadata << m_cur_spill_dir_integer << "/" << data_id << " " << data_id
             << std::endl;
  m_spilled_nodes[data_id] = m_cur_spill_dir_integer;
  ...
  for (const auto& v : m_indices_to_send) {
    for (const auto& id : v) {
      ...
      if (it == m_spilled_nodes.end()) {
        LBANN_ERROR("it == m_spilled_nodes.end() for sample_id: ",
                    id,
                    "; m_spilled_nodes.size: ",
                    m_spilled_nodes.size());
      }
      const std::string fn = get_conduit_dir() + "/" +
...
void data_store_conduit::open_informational_files()
{
  ...
  if (arg_parser.get<bool>(LBANN_OPTION_DATA_STORE_DEBUG) && !m_debug &&
      m_reader != nullptr) {
    m_debug_filename = m_debug_filename_base + "_" + m_reader->get_role() +
  ...
  if (arg_parser.get<bool>(LBANN_OPTION_DATA_STORE_PROFILE) && m_world_master &&
    ...
    m_profile_filename =
      m_profile_filename_base + "_" + m_reader->get_role() + ".txt";
...
{
  std::cout << "\nHere is part of the owner map; m_owner.size(): "
            << m_owner.size() << std::endl;
  ...
  int j = 0;
  for (auto t : m) {
    std::cout << "  sample_id: " << t.first.first << ":" << t.first.second
              << " owner: " << t.second << std::endl;
    if (j++ >= 10)
      break;
  }
}

void data_store_conduit::set_profile_msg(std::string s) { PROFILE(s); }
...
void data_store_conduit::test_imagenet_node(int index, bool dereference)
{
  ...
  PROFILE("test_imagenet_node() for data_id: ",
          utils::commify(data_id),
          " at offset: ",
          utils::commify(m_image_offsets[data_id]),
          " image size: ",
          utils::commify(szz));
  ...
  std::cout << "testing sample_id: " << utils::commify(data_id)
            << " stored at offset: "
            << utils::commify(m_image_offsets[data_id]);
  if (m_image_offsets[data_id] >= INT_MAX) {
    std::cout << "; (>= INT_MAX)\n";
  ...
  if (size1 != size2) {
    PROFILE("buffer sizes mismatch: size of buffer read from file does not "
            "match buffer size from cache; from file: ",
            size1,
            " from cache: ",
            size2,
            " for data_id: ",
    ...
    LBANN_ERROR("buffer sizes mismatch: size of buffer read from file does not "
                "match buffer size from cache; from file: ",
                size1,
                " from cache: ",
                size2,
  ...
  for (size_t i = 0; i < size1; i++) {
    if (buf1[i] != buf2[i]) {
      PROFILE("buffer mismatch for char #",
              i + 1,
              " of ",
              size1,
              "; image buffer read from file does not match buffer from "
              "conduit node");
      LBANN_ERROR("buffer mismatch for char #",
                  i + 1,
                  " of ",
                  size1,
                  "; image buffer read from file does not match buffer from "
...
void data_store_conduit::check_query_flags() const
{
  if (m_explicitly_loading && m_preloading) {
    LBANN_ERROR("is_explicitly_loading() && is_preloading() are both true, but "
                "should not be");
  }
  if (m_loading_is_complete && m_explicitly_loading) {
    LBANN_ERROR("is_fully_loaded() && is_explicitly_loading() are both true, "
                "but should not be");
  }
  if (m_loading_is_complete && m_preloading) {
    LBANN_ERROR(
      "is_fully_loaded() && is_preloading() are both true, but should not be");
  }
}
...
void data_store_conduit::verify_sample_size()
{
  ...
  int max_samples =
    m_comm->trainer_allreduce<int>(m_compacted_sample_size, El::mpi::MAX);
  if (max_samples <= 0) {
    LBANN_ERROR("sample size, which is needed for data exchange, is invalid; "
                "should be > 0, but value is: ",
                max_samples,
                "; this indicates there is insufficient data. Role: ",
                m_reader->get_role());
  }
  if (m_compacted_sample_size != 0 && max_samples != m_compacted_sample_size) {
    LBANN_ERROR("m_compacted_sample_size = ",
                m_compacted_sample_size,
                " but max_samples = ",
                max_samples,
