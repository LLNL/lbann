...
__global__ void kfac_get_diagonal_kernel(TensorDataType* __restrict__ diag,
...
template <typename TensorDataType>
__global__ void kfac_conv_transpose_kernel(const TensorDataType* __restrict__ A,
                                           ...
                                           const size_t num_elems)
{
  ...
  if (gid < num_elems) {
    ...
    Acol[i_c + i_spatial * num_channels + i_n * num_channels * spatial_prod] =
      A[gid];
  }
}
  ...
  const El::SyncInfo<El::Device::GPU>& sync_info)
{
  ...
  hydrogen::gpu::LaunchKernel(kfac_get_diagonal_kernel<DataType>,
                              grid_size,
                              block_size,
                              0,
                              sync_info,
  ...
  const El::SyncInfo<El::Device::GPU>& sync_info)
{
  ...
  if (grid_size > 0) {
    hydrogen::gpu::LaunchKernel(kfac_conv_transpose_kernel<DataType>,
                                grid_size,
                                block_size,
                                0,
                                sync_info,
