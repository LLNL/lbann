...
# Basic test function
def _test_func(cluster, weekly, executables, dir_name, compiler_name):
...
def _test_func_exe(cluster, weekly, dirname, exe):
    if exe is None:
        e = '{}_exe: Non-local testing'.format(_test_name)
        print('Skip - ' + e)
        pytest.skip(e)
    exes = {'exe': exe}
    _test_func(cluster, weekly, exes, dirname, 'exe')
def _test_func_clang6(cluster, weekly, exes, dirname):
    _test_func(cluster, weekly, exes, dirname, 'clang6')
def _test_func_gcc7(cluster, weekly, exes, dirname):
    _test_func(cluster, weekly, exes, dirname, 'gcc7')
def _test_func_intel19(cluster, weekly, exes, dirname):
    _test_func(cluster, weekly, exes, dirname, 'intel19')
