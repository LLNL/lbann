...
def test_compiler_build_script(cluster, dirname):
    ...
    if cluster in ['lassen', 'pascal', 'ray']:
        command = '%s/scripts/build_lbann.sh -d -l bamboo --test --clean-build -j $(($(nproc)+2)) -- +cuda +deterministic +half +fft +vision +numpy > %s 2> %s' % (
            dirname, output_file_name, error_file_name)
    elif cluster in ['corona']:
        command = '%s/scripts/build_lbann.sh -d -l bamboo --test --clean-build -j $(($(nproc)+2)) -- +rocm +deterministic +vision +numpy > %s 2> %s' % (
            dirname, output_file_name, error_file_name)
    elif cluster in ['catalyst']:
        command = '%s/scripts/build_lbann.sh -d -l bamboo --test --clean-build -j $(($(nproc)+2)) -- +onednn +deterministic +half +fft +vision +numpy > %s 2> %s' % (
            dirname, output_file_name, error_file_name)
    else:
