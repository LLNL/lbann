...
""" % ((tab,)*3))
  ...
  (input_name, parent) = initial_layer(f, tab, number_initial_features)
  num_blocks = 4
  for num in range(1, num_blocks + 1):
    num_layers = num_layers_tuple[num - 1]
    parent = dense_block(f, tab, num, num_layers, parent, number_initial_features, batch_norm_size, growth_rate)
    if num != num_blocks:
  ...
  classification_layer(f, tab, parent, input_name)
  f.write('}\n')

def initial_layer(f, tab, num_initial_channels):
  name = 'initial_layer'
  input_name = generator_base.input_layer(f, tab, name, [], io_buffer='partitioned')
  ...
  conv_block_name = conv_block_initial(f, tab, name, [input_name], num_dims=2, num_output_channels=num_initial_channels, conv_dims_i=7, conv_pads_i=3, conv_strides_i=2)
  ...
  pool_name = generator_base.pool(f, tab, name, [conv_block_name], num_dims=2, pool_dims_i=3, pool_pads_i=1, pool_strides_i=2, pool_mode='max')
  return (input_name, pool_name)

def conv_block_initial(f, tab, name, parents, num_dims=None, num_output_channels=None, conv_dims_i=None, conv_pads_i=None, conv_strides_i=None):
  convolution_name = generator_base.convolution(f, tab, name, parents, num_dims=num_dims, num_output_channels=num_output_channels, conv_dims_i=conv_dims_i, conv_pads_i=conv_pads_i, conv_strides_i=conv_strides_i, has_bias=False)
  norm_name = generator_base.norm(f, tab, name, [convolution_name], decay='0.9', scale_init='1.0', bias_init='0.0', epsilon='1e-5')
  relu_name = generator_base.relu(f, tab, name, [norm_name])
  return relu_name

def dense_block(f, tab, num, num_layers, parent, num_initial_channels, batch_norm_size, growth_rate):
  ...
  parent_name = dense_layer(f, tab, name, layer_num, [parent], batch_norm_size, growth_rate, num_output_channels + ((layer_num-1)*growth_rate))
  # Parents of following dense_layer are the preceding dense_layers.
  parents = [parent_name]
  for layer_num	in range(2, num_layers + 1):
    parent_name = dense_layer(f, tab, name, layer_num, parents, batch_norm_size, growth_rate, num_output_channels + ((layer_num-1)*growth_rate))
    parents.append(parent_name)
  return parent_name # Returns the name of the last dense_layer.

def dense_layer(f, tab, name, num, parents, batch_norm_size, growth_rate, num_output_channels):
  complete_name = '%s_dense_layer_%d' % (name, num)
  concatenation_name = generator_base.concatenation(f, tab, complete_name, parents)
  ...
  part2_name = conv_block(f, tab, complete_name + '_part_2', [part1_name], num_dims=2, num_output_channels=num_output_channels, conv_dims_i=3, conv_pads_i=1, conv_strides_i=1)
  return part2_name
...
def conv_block(f, tab, name, parents, num_dims=None, num_output_channels=None, conv_dims_i=None, conv_pads_i=None, conv_strides_i=None):
  norm_name = generator_base.norm(f, tab, name, parents, decay=0.9, scale_init=1.0, bias_init=0.0, epsilon=1e-5)
  relu_name = generator_base.relu(f, tab, name, [norm_name])
  convolution_name = generator_base.convolution(f, tab, name, [relu_name], num_dims=num_dims, num_output_channels=num_output_channels, conv_dims_i=conv_dims_i, conv_pads_i=conv_pads_i, conv_strides_i=conv_strides_i, has_bias=False)
...
def transition_layer(f, tab, num, parent, num_output_channels):
  ...
  pool_name = generator_base.pool(f, tab, name, [conv_block_name], num_dims=2, pool_dims_i=2, pool_strides_i=2, pool_mode='average')
  return pool_name

def classification_layer(f, tab, parent, input_name):
  name = 'classification_layer'
  # pool_dims_i=7, pool_mode='average' specified in "Densely Connected Convolutional Networks"
  pool_name = generator_base.pool(f, tab, name, [parent], num_dims=2, pool_dims_i=7, pool_strides_i=1, pool_mode='average')
  fc_name = generator_base.fully_connected(f, tab, name, [pool_name], num_neurons=1000, has_bias=False)
  softmax_name = generator_base.softmax(f, tab, name, [fc_name])
  generator_base.target(f, tab, name, [softmax_name, input_name], io_buffer='partitioned', shared_data_reader=True)
