...
def setup_layers(model):
    for i in model.layers:
        ...
        if hasattr(i,'activation') and i.activation.__name__ and not explicit_activation:
            # this is real hacky
            l = None
            activation(i,l)
...
def reshape(keras_layer, pb_layer):
    ...
    if 'flatten' in keras_layer.name.lower():
        pb_layer.reshape.num_dims = 1
        pb_layer.reshape.dims = str(keras_layer._inbound_nodes[0].output_shapes[0][1])
    else:
