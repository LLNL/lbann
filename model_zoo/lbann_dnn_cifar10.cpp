...
//
// lbann_dnn_cifar10.cpp - DNN application for CIFAR-10 classification
////////////////////////////////////////////////////////////////////////////////

#include "core/lbann_model_dnn.hpp"
#include "core/lbann_params.hpp"
#include "core/lbann_io.h"
#include "tools/imagenet.h"
...
void getTrainDataMB(CImageNet& ImageNet, const int* Indices, unsigned char* ImageBuf, Mat& X, Mat& Y, int MBSize, int inputDim)
{
    ...
    for (uint k = 0; k < MBSize; k++) {
        ...
        // printf("Loading image at index Indices[%d]=%d label %d\n", k, Indices[k], imagelabel);
...
bool checkpointShared(int epoch, int train, vector<int>& indices, TrainingParams& trainParams, CDnn* dnn)
{
...
bool restartShared(int* epochStart, int* trainStart, vector<int>& indices, TrainingParams& trainParams, CDnn* dnn)
{
    // create top level directory
    const char* dir = trainParams.ParameterDir.c_str();
...
bool checkpoint(int epoch, int train, vector<int>& indices, TrainingParams& trainParams, CDnn* dnn)
{
    // time how long this takes
    Timer timer;
...
bool restart(int* epochStart, int* trainStart, vector<int>& indices, TrainingParams& trainParams, CDnn* dnn)
{
    // create top level directory
    const char* dir = trainParams.ParameterDir.c_str();
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        CDnn *dnn = NULL; 
        CAutoEncoder *autoencoder = NULL;
        if (g_AutoEncoder)
          autoencoder = new CAutoEncoder(netParams.Network, false, trainParams.MBSize, trainParams.ActivationType, trainParams.DropOut, trainParams.Lambda, grid);
        else 
          dnn = new CDnn(netParams.Network, trainParams.MBSize, trainParams.ActivationType, trainParams.DropOut, trainParams.Lambda, grid);
        ...
        if (! restarted) {
            ...
            int tmpNumTrainData = imagenet.getNumTrainData()-1;
...
#if 0
                  if(n + r * trainParams.MBSize > decayIterations * trainParams.LrDecayCycles) {
                    trainParams.LearnRate = trainParams.LearnRate * trainParams.LrDecayRate;
                    decayIterations++;
                    if(grid.Rank() == 0) {
                      cout << "Changing the learning rate to " << trainParams.LearnRate << " after processing " << (n + r * trainParams.MBSize) << " dataums" << endl;
                    }
                  }
#endif
                  mpi::Barrier(grid.Comm());
