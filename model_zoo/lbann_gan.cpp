...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    auto model_1 = build_model_from_prototext(argc, argv, pb_trainer, *(pbs[0]), comm.get(), opts, trainer->get_name(), io_thread_pool,
                                              trainer->get_callbacks(), true); //discriminator
                                                                                    //model
    std::unique_ptr<model> model_2 = nullptr; //adversarial model
    if (pbs.size() > 1) {
      model_2 = build_model_from_prototext(argc, argv, pb_trainer, *(pbs[1]), comm.get(), opts, trainer->get_name(), io_thread_pool,
